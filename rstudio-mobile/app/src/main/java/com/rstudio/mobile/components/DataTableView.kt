...
fun DataTableView(table: DataTable?) {
    ...
    Column(Modifier.fillMaxSize()) {
        Column(Modifier.fillMaxWidth().padding(12.dp)) {
            ...
            Text(
                if (shown < table.totalRows) "Showing first $shown of ${table.totalRows} rows" else "${table.totalRows} rows",
        ...
        Column(Modifier.fillMaxSize().horizontalScroll(horizontal)) {
            TableRow(cells = listOf("#") + table.columns, header = true)
            HorizontalDivider()
            LazyColumn(Modifier.fillMaxSize()) {
                itemsIndexed(table.rows) { index, row ->
                    TableRow(cells = listOf((index + 1).toString()) + row, header = false)
...
private fun TableRow(cells: List<String>, header: Boolean) {
    Row(
        ...
        cells.forEachIndexed { index, cell ->
            Text(
                text = cell,
                modifier = Modifier.width(if (index == 0) 56.dp else 144.dp).padding(end = 10.dp),
                maxLines = 1,
