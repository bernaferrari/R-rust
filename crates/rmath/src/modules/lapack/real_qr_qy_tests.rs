//! Direct safety contracts for the real QR Q*y embedding boundary.
use std::panic::{AssertUnwindSafe, catch_unwind};

use super::lapack_impl::qr_qy_real;
...
#[test]
fn real_qr_qy_rejects_malformed_dims_and_payload() {
    let session = RSession::new();
    session.with_active(|| unsafe {
        let qr = make_qr(
            make_matrix(&[2, 2], &[1.0, 0.0, 0.0, 1.0]),
            make_real_vector(&[0.0, 0.0]),
        );
        let _qr = protect(qr);
...
#[test]
fn real_qr_qy_reserves_caller_scratch_before_allocation_and_recovers() {
