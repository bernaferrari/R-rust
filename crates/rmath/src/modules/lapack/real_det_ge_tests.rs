//! Direct safety contracts for the real determinant embedding boundary.
...
fn error_message(result: Result<(), Box<dyn std::any::Any + Send>>) -> String {
    result
        .expect_err("invalid determinant call should fail")
...
#[test]
fn real_det_ge_rejects_malformed_dims_and_payload() {
    let session = RSession::new();
    session.with_active(|| unsafe {
        let logarithm = Rf_ScalarLogical(0);
...
#[test]
fn real_det_ge_reserves_caller_scratch_before_allocation_and_recovers() {
    let mut session = RSession::new();
    let (input, logarithm) = session.with_active(|| unsafe {
        (
    ...
    session.with_active(|| unsafe {
        let _input = protect(input);
        let _logarithm = protect(logarithm);
        let message = error_message(catch_unwind(AssertUnwindSafe(|| {
            det_ge_real(input, logarithm);
        })));
        assert!(
            message.contains("native determinant workspace exceeds resource limit"),
            "{message}"
        );
    });
