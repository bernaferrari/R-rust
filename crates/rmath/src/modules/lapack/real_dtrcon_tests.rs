//! Direct safety contracts for the real triangular condition-number boundary.
use std::panic::{AssertUnwindSafe, catch_unwind};

use super::lapack_impl::La_dtrcon;
...
#[test]
fn real_dtrcon_accepts_zero_dimensional_square() {
...
#[test]
fn real_dtrcon_rejects_malformed_dims_and_payload() {
    let session = RSession::new();
    session.with_active(|| unsafe {
        let norm = one_norm();
        let _n = protect(norm);
...
#[test]
fn real_dtrcon_reserves_caller_scratch_before_allocation_and_recovers() {
    ...
    session.with_active(|| unsafe {
        ...
        assert!(
            message.contains("native triangular condition-number workspace exceeds resource limit"),
            "{message}"
        );
    });
