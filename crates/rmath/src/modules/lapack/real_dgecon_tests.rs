//! Direct safety contracts for the real condition-number embedding boundary.
use std::panic::{AssertUnwindSafe, catch_unwind};

use super::lapack_impl::La_dgecon;
...
#[test]
fn real_dgecon_rejects_malformed_dims_and_payload() {
    let session = RSession::new();
    session.with_active(|| unsafe {
        let norm = one_norm();
        let _n = protect(norm);
...
fn real_dgecon_reserves_caller_scratch_before_allocation_and_recovers() {
    ...
    session.with_active(|| unsafe {
        ...
        assert!(
            message.contains("native condition-number workspace exceeds resource limit"),
            "{message}"
        );
    });
