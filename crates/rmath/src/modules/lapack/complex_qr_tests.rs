//! Direct safety contracts for the complex QR embedding boundary.
use std::panic::{AssertUnwindSafe, catch_unwind};

use super::lapack_impl::La_qr_cmplx;
...
unsafe fn make_input(dimensions: &[i32], data_len: usize) -> SEXP {
    unsafe {
        let data = Rf_allocVector3(SEXPTYPE::CPLXSXP, data_len as R_xlen_t);
        ...
        // Deliberately bypass attribute validation to test the unsafe boundary.
        let attrs = Rf_cons(dims, R_NilValue());
        SETTAG(attrs, R_DimSymbol());
        SET_ATTRIB(data, attrs);
        for i in 0..data_len {
            (*COMPLEX(data).add(i)).r = i as f64 + 1.0;
        }
        data
    }
}
...
#[test]
fn complex_qr_accepts_zero_dimensional_shapes() {
...
#[test]
fn complex_qr_rejects_short_dimensions_and_payload() {
    let session = RSession::new();
    session.with_active(|| unsafe {
...
#[test]
fn complex_qr_reserves_caller_scratch_before_allocation_and_recovers() {
    let mut session = RSession::new();
    let input = session.with_active(|| unsafe { make_input(&[2, 1], 2) });
    // No R allocation occurs between construction and rooting in with_active.
