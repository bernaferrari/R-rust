 ...
 *
 *  Ported to Rust for the crates project.
...
use crate::sexp::ffi::{FALSE, NA_LOGICAL, R_xlen_t, SEXP, SEXPTYPE, TRUE};
    ...
    unsafe {
        ...
        for i in 0..n as usize {
            ...
            while byte_idx < s_bytes.len() {
                ...
                } else if byte_idx + lend <= s_bytes.len()
                    && s_bytes[byte_idx..byte_idx + lend] == *de
                {
                    if delim_depth > 1 {
                        delim_depth -= 1;
...
pub unsafe fn doTabExpand(strings: SEXP, starts: SEXP) -> SEXP {
    unsafe {
        ...
        for i in 0..n as usize {
            ...
            let start_val = if !starts_ptr.is_null() {
                *starts_ptr.add(i)
            } else {
                0
            };
