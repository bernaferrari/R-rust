...
unsafe fn snapshot_portable_state(recording: SEXP) {
    unsafe {
        let grid_symbol = Rf_install(GRID_STATE_ATTR.as_ptr() as *const _);
        if let Some(bytes) =
            with_required_current_instance(|inst| (*inst).portable_grid.encode_snapshot().ok())
        {
            let grid = Rf_allocVector3(SEXPTYPE::RAWSXP, bytes.len() as i64);
            let _grid_guard = protect(grid);
            std::ptr::copy_nonoverlapping(bytes.as_ptr(), RAW(grid), bytes.len());
            setAttrib(recording, grid_symbol, grid);
        }
        let Some(coords) =
...
unsafe fn decode_grid_state(
    recording: SEXP,
    source: (u32, u32),
    target: (u32, u32),
) -> Result<Option<crate::mainutils::portable_grid::GridState>, &'static str> {
    unsafe {
        let symbol = Rf_install(GRID_STATE_ATTR.as_ptr() as *const _);
        let value = getAttrib(recording, symbol);
        if value == R_NilValue() {
            return Ok(None);
        }
        if TYPEOF(value) != SEXPTYPE::RAWSXP {
            return Err("recorded plot has invalid grid state metadata");
        }
        let bytes = std::slice::from_raw_parts(RAW(value), XLENGTH(value) as usize);
        let mut state = crate::mainutils::portable_grid::GridState::decode_snapshot(bytes)?;
        state.scale_snapshot(source, target)?;
        Ok(Some(state))
    }
}
...
pub(crate) unsafe fn replay(_: SEXP, _: SEXP, args: SEXP, _: SEXP) -> SEXP {
    unsafe {
        ...
        let grid_state = decode_grid_state(recording, source_dimensions, target_dimensions)
        ...
        if let Some(grid_state) = grid_state {
            with_required_current_instance(|inst| (*inst).portable_grid = grid_state);
        }
