// Ported 1:1 from ./r-source/src/extra/graphapp/dialogs.c - original code structure preserved
...
pub unsafe fn setuserfilter(const char *uf) {
{
    userfilter=uf;
}

pub unsafe fn setuserfilterW(const wchar_t *uf) {
{
    userfilterW=uf;
}
...
pub unsafe fn myMessageBox(HWND h, const char *text, const char *caption, UINT type) -> c_int {
{
    if(localeCP != GetACP()) {
 ...
 */
pub unsafe fn apperror(const char *errstr) {
{
    if (! errstr)
...
pub unsafe fn askok(const char *info) {
{
    if (! info)
	info = "";
...
pub unsafe fn askokcancel(const char *question) -> c_int {
{
    int result;
...
pub unsafe fn askyesno(const char *question) -> c_int {
{
    int result;
...
pub unsafe fn askyesnocancel(const char *question) -> c_int {
{
    int result;
...
/* returns 0 on error */
static pub unsafe fn savecod(void) -> c_int {
{
...
pub unsafe fn askchangedir(void) {
{
    char *s, *msg;
...
pub unsafe fn countFilenames(const char *list) -> c_int {
{
    const char *temp;
    int count;
    count = 0;
...
static pub unsafe fn add_data(window w) {
{
    dialog_data *d;
...
static pub unsafe fn hit_button(control c) {
{
...
static pub unsafe fn hit_key(window w, int key) {
{
    button btn;
    char *name = NULL;
 ...
 */
static pub unsafe fn handle_message_dialog(window w) -> c_int {
{
    window old;
    dialog_data *d = data(w);
...
pub unsafe fn clickbutton(window w, button b) {
    sendmessage(w->handle, WM_COMMAND,
	(BN_CLICKED << 16) | GetWindowLong(b->handle, GWL_ID),
	b->handle);
}

static pub unsafe fn askstr_dialog_close(window win) {
{
    dialog_data *d;
...
pub unsafe fn modeless_active(void) -> c_int {
{
    if (hModelessDlg)
	return 1;
    return 0;
...
pub unsafe fn finddialog(textbox t) {
{
...
pub unsafe fn replacedialog(textbox t) {
{
    static FINDREPLACE fr;
    static char szFindWhat[80];
...
static pub unsafe fn richeditfind(HWND hwnd, char *what, pub unsafe fn matchcase,
...
static pub unsafe fn richeditreplace(HWND hwnd, char *what, char *replacewith,
...
PROTECTED
pub unsafe fn handle_findreplace(HWND hwnd, LPFINDREPLACE pfr) {
{
    CHARRANGE sel;
