// Ported 1:1 from ./r-source/src/extra/graphapp/image.c - original code structure preserved
 ...
 *  Delete an image:
 */
pub unsafe fn delimage(image img) {
{
    if (img) {
	discard(img->cmap);
	discard(img->pixels);
...
#if DEBUG
PROTECTED
pub unsafe fn printimage(FILE *file, image img) {
{
    int i;
    long w, h, n;
...
pub unsafe fn imagedepth(image img)  { return ((img)?(img->depth):0);}
pub unsafe fn imagewidth(image img)  { return ((img)?(img->width):0);}
pub unsafe fn imageheight(image img) { return ((img)?(img->height):0);}

/*
 *  Change an image's pixels:
 */
pub unsafe fn setpixels(image img, GAbyte pixels[]) {
{
    long i, length;
 ...
 */
pub unsafe fn setpalette(image img, int cmapsize, rgb *cmap) {
{
    int i;
...
pub unsafe fn getpalettesize(image img) -> c_int {
{
    if (img)
	return img->cmapsize;
    else
...
static pub unsafe fn compare_freq(long *a, long *b) -> c_int {
{
    long freq_a, freq_b;
    int value_a, value_b;
...
pub unsafe fn sortpalette(image img) {
{
    long	i, j, length;
 ...
 */
static pub unsafe fn string_ends_with(const char *name, const char *ending) -> c_int {
{
    int i, j, result;
...
static pub unsafe fn save_header_image_file(FILE *file, char *name, image img) {
{
    long i, size;
    int width;
    rgb * pixel32;
...
static pub unsafe fn save_header_image(image img, const char *filename) {
{
    FILE *file;
    char *name;
...
pub unsafe fn saveimage(image img, const char *filename) {
{
    if (string_ends_with(filename, ".gif"))
...
static pub unsafe fn scale_8_bit_image(image dest, image src, rect dr, rect sr) {
{
    int value;
    long x, y;
    long dx, dy, sx, sy;
...
static pub unsafe fn scale_32_bit_image(image dest, image src, rect dr, rect sr) {
{
    rgb value;
    long x, y;
    long dx, dy, sx, sy;
...
static pub unsafe fn get_mono_pixval(image src, image dest, pub unsafe fn x, pub unsafe fn y) -> c_int {
{
    int i;
...
static pub unsafe fn get_grey_pixval(image src, image dest, pub unsafe fn x, pub unsafe fn y) -> c_int {
{
    int i;
...
pub unsafe fn drawmonochrome(image src, rect dr, rect sr) {
{
    image dest;
    int x, y;
...
pub unsafe fn drawgreyscale(image src, rect dr, rect sr) {
{
    image dest;
    int x, y;
...
pub unsafe fn drawdarker(image src, rect dr, rect sr) {
{
    image dest = NULL;
    int i, x, y;
...
pub unsafe fn drawbrighter(image src, rect dr, rect sr) {
{
    image dest = NULL;
    int i, x, y;
    rgb *newcmap = NULL, *oldcmap = NULL;
