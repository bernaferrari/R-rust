// Ported 1:1 from ./r-source/src/extra/graphapp/gmenus.c - original code structure preserved
...
static pub unsafe fn mdimenu(menuitem m) {
{
    switch (getvalue(m)) {
    case 1:
...
pub unsafe fn gchangepopup(window w, menu m) {
{
    w->popup = m;
}

pub unsafe fn gchangemenubar(menubar mb) {
{
...
static pub unsafe fn addmenuitemarray(menu m,MenuItem a[]) -> c_int {
{
    menu ma  = m;
    int i = 0;
    while (a[i].nm) {
