// Ported 1:1 from ./r-source/src/extra/graphapp/gdraw.c - original code structure preserved
...
pub unsafe fn gsetcliprect(drawing d, rect r) {
{
    HRGN rgn;
    HDC dc = GETHDC(d);
...
pub unsafe fn gbitblt(drawing db, drawing sb, point p, rect r) {
{
    HDC src;
    HDC dst;
...
pub unsafe fn gscroll(drawing d, point dp, rect r) {
{
    HDC dc = GETHDC(d);
    RECT rr ;
    rr.left = r.x;
...
pub unsafe fn ginvert(drawing d, rect r) {
{
    HDC dc = GETHDC(d);
...
pub unsafe fn gsetpixel(drawing d, point p, rgb c) {
{
    HDC dc = GETHDC(d);
...
pub unsafe fn gdrawline(drawing d, int width, int style, rgb c, point p1, point p2,
...
pub unsafe fn gdrawpolyline(drawing d, int width, int style, rgb c,
...
pub unsafe fn gdrawrect(drawing d, int width, int style, rgb c, rect r, int fast,
	       int lend, int ljoin, float lmitre)
{
    int x0 = r.x;
    int y0 = r.y;
...
pub unsafe fn gfillrect(drawing d, rgb fill, rect r) {
{
    HDC dc = GETHDC(d);
...
pub unsafe fn gcopy(drawing d, drawing d2, rect r) {
{
    HDC dc = GETHDC(d), sdc = GETHDC(d2);
    BitBlt(dc, r.x, r.y, r.width, r.height, sdc, r.x, r.y, SRCCOPY);
}

pub unsafe fn gcopyalpha(drawing d, drawing d2, rect r, int alpha) 
{
    if(alpha <= 0) return;
    {
	BLENDFUNCTION bl;
...
pub unsafe fn gcopyalpha2(drawing d, image src, rect r) 
{
    BLENDFUNCTION bl;
    bl.BlendOp = AC_SRC_OVER;
    bl.BlendFlags = 0;
...
pub unsafe fn gdrawellipse(drawing d, int width, rgb border, rect r, int fast,
		  int lend, int ljoin, float lmitre)
{
    HDC dc = GETHDC(d);
    LOGBRUSH lb;
...
pub unsafe fn goldfillellipse(drawing d, rgb fill, rect r) {
{
    HDC dc = GETHDC(d);
    HBRUSH br = CreateSolidBrush(getwinrgb(d, fill));
    fix_brush(dc, d, br);
...
pub unsafe fn gfillellipse(drawing d, rgb fill, rect r) {
...
pub unsafe fn gsetpolyfillmode(drawing d, int oddeven) {
{
    HDC dc = GETHDC(d);
    SetPolyFillMode(dc, oddeven ? ALTERNATE : WINDING);
}

pub unsafe fn gfillpolygon(drawing d, rgb fill, point *p, int n) {
{
    HDC dc = GETHDC(d);
    HBRUSH br = CreateSolidBrush(getwinrgb(d,fill));
    fix_brush(dc, d, br);
...
pub unsafe fn gfillpolypolygon(drawing d, rgb fill, point *p, int npoly, int *nper) {
{
    HDC dc = GETHDC(d);
    HBRUSH br = CreateSolidBrush(getwinrgb(d,fill));
    fix_brush(dc, d, br);
 ...
 */
pub unsafe fn gdrawimage(drawing d, image img, rect dr, rect sr) {
{
    HDC dc = GETHDC(d);
    HDC bc;
    bitmap b;
 ...
 */
pub unsafe fn gmaskimage(drawing d, image img, rect dr, rect sr, image mask) {
{
    HDC dc = GETHDC(d);
    HDC bc, mbc, mbwc;
    bitmap b, mb, mbw;
...
pub unsafe fn gdrawstr(drawing d, font f, rgb c, popub unsafe fn p, const char *s) -> c_int {
{
    POINT curr_pos;
    int width;
    HFONT old;
...
pub unsafe fn gdrawwcs(drawing d, font f, rgb c, popub unsafe fn p, const wchar_t *s) -> c_int {
{
    POINT curr_pos;
    int width;
    HFONT old;
...
pub unsafe fn gdrawstr1(drawing d, font f, rgb c, point p, const char *s, double hadj) {
{
    HFONT old;
    HDC dc = GETHDC(d);
...
/* widechar version */
pub unsafe fn gwdrawstr1(drawing d, font f, rgb c, point p,
...
popub unsafe fn gstrsize(drawing d, font f, const char *s) -> c_int {
{
    rect r = gstrrect(d, f, s);
    return pt(r.width, r.height);
}

pub unsafe fn gstrwidth(drawing d, font f, const char *s) -> c_int {
{
    rect r = gstrrect(d, f, s);
    return r.width;
}
...
pub unsafe fn gwcswidth(drawing d, font f, const wchar_t *s) -> c_int {
{
    rect r = gwcsrect(d, f ,s);
    return r.width;
}

pub unsafe fn gstrwidth1(drawing d, font f, const char *s, pub unsafe fn enc) -> c_int {
{
    rect r;
    if (enc == CE_UTF8) {
	wchar_t *wc;
...
pub unsafe fn ghasfixedwidth(font f) -> c_int {
{
    TEXTMETRIC tm;
    HFONT old;
    HDC dc = GetDC(0);
...
pub unsafe fn gcharmetric(drawing d, font f, int c, int *ascent, int *descent,
		 int *width)
{
...
pub unsafe fn gwcharmetric(drawing d, font f, int c, int *ascent, int *descent,
		  int *width)
{
    int first, last, extra;
    TEXTMETRICW tm;
...
static pub unsafe fn measuredev(drawing dev, pub unsafe fn what) -> c_int {
{
    HDC hDC;
    int n;
    if (dev)
...
pub unsafe fn devicewidth(drawing dev) MEASUREDEV(HORZRES) -> c_int {
pub unsafe fn deviceheight(drawing dev) MEASUREDEV(VERTRES) -> c_int {
pub unsafe fn devicewidthmm(drawing dev) MEASUREDEV(HORZSIZE) -> c_int {
pub unsafe fn deviceheightmm(drawing dev) MEASUREDEV(VERTSIZE) -> c_int {
pub unsafe fn devicepixelsx(drawing dev) MEASUREDEV(LOGPIXELSX) -> c_int {
pub unsafe fn devicepixelsy(drawing dev) MEASUREDEV(LOGPIXELSY) -> c_int {

pub unsafe fn isTopmost(window c) -> c_int {
{
    return GetWindowLong(c->handle, GWL_EXSTYLE) & WS_EX_TOPMOST;
}

static pub unsafe fn setMessageBoxTopmost(window obj) {
{
...
pub unsafe fn BringToTop(window c, int stay) /* stay=0 for regular, 1 for topmost, 2 for toggle */
{
...
/* type = 1 minimize
	  ...
	  4 hide
*/
pub unsafe fn GA_msgWindow(window c, int type) {
{
    int state = -1;
    switch(type){
