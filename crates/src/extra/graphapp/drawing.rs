// Ported 1:1 from ./r-source/src/extra/graphapp/drawing.c - original code structure preserved
...
pub unsafe fn move_to_ex(HDC hdc, pub unsafe fn x, pub unsafe fn y, POINT *p);
pub unsafe fn get_current_position_ex(HDC hdc, POINT *p);
pub unsafe fn get_text_extent_point(HDC hdc, const char *str, pub unsafe fn len, SIZE *s);

static pub unsafe fn move_to_ex(HDC hdc, pub unsafe fn x, pub unsafe fn y, POINT *p) -> c_int {
{
...
static pub unsafe fn get_current_position_ex(HDC hdc, POINT *p) -> c_int {
{
...
static pub unsafe fn get_text_extent_point(HDC hdc, const char *str, pub unsafe fn len, SIZE *s) -> c_int {
{
...
pub unsafe fn setcliprect(rect r) {
{
    HRGN rgn;
 ...
 */
PROTECTED
pub unsafe fn fix_brush(HDC dc, drawing obj, HBRUSH brush) {
{
    POINT p;
    HWND hwnd;
    drawing parent;
 ...
 */
static pub unsafe fn enable_drawing(void) {
{
    if (! current->dest) {
	if (! current_window)
 ...
 */
pub unsafe fn bitblt(bitmap db, bitmap sb, point p, rect r, int mode) {
{
    HDC src;
    HDC dst;
...
pub unsafe fn scrollrect(point dp, rect r) {
{
    rect cliprect;
...
pub unsafe fn copyrect(bitmap sb, point p, rect r) {
{
    enable_drawing();
    if (current->dest) bitblt(current->dest, sb, p, r, GA_S);
}

pub unsafe fn texturerect(bitmap sb, rect dr) {
{
...
pub unsafe fn invertrect(rect r) {
{
    enable_drawing();
...
pub unsafe fn setpixel(point p, rgb c) {
{
    rgb old = current->hue;
...
pub unsafe fn moveto(point p) {
{
    current->p = p;
}

pub unsafe fn lineto(point p) {
{
    drawline(current->p, p);
    moveto(p);
}

pub unsafe fn drawpoint(point p) {
{
    setpixel(p, current->hue);
}

pub unsafe fn drawline(point p1, point p2) {
{
...
pub unsafe fn drawarc(rect r, int start_angle, int end_angle) {
{
    point p0, p1, p2;
    double start, end;
    int fudge;
...
pub unsafe fn fillarc(rect r, int start_angle, int end_angle) {
{
    point p0, p1, p2;
    double start, end;
...
pub unsafe fn drawellipse(rect r) {
{
    enable_drawing();
    SelectObject(dc, the_pen);
    SetROP2(dc, pen_mode[current->mode]);
 ...
 */
pub unsafe fn oldfillellipse(rect r) {
{
    enable_drawing();
    SelectObject(dc, the_brush);
    SetROP2(dc, pen_mode[current->mode]);
...
pub unsafe fn fillellipse(rect r) {
...
pub unsafe fn drawroundrect(rect r) {
{
    int minimum, radius;
...
pub unsafe fn fillroundrect(rect r) {
{
    int minimum, radius;
...
pub unsafe fn drawpolygon(point *p, int n) {
{
    enable_drawing();
    SelectObject(dc, the_pen);
    SetROP2(dc, pen_mode[current->mode]);
...
pub unsafe fn fillpolygon(point *p, int n) {
{
    enable_drawing();
    SelectObject(dc, the_brush);
    SetROP2(dc, pen_mode[current->mode]);
 ...
 */
pub unsafe fn drawstr(popub unsafe fn p, const char *s) -> c_int {
{
    POINT curr_pos;
    int width;
    HFONT old;
...
popub unsafe fn strsize(font f, const char *s) -> c_int {
{
    rect r = strrect(f,s);
    return pt(r.width, r.height);
}

pub unsafe fn strwidth(font f, const char *s) -> c_int {
{
    rect r = strrect(f,s);
    return r.width;
}
 ...
 *  Draw an image:
 */
pub unsafe fn drawimage(image img, rect dr, rect sr) {
{
    bitmap b;
    image i = img;
