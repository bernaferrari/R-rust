// Ported 1:1 from ./r-source/src/extra/graphapp/init.c - original code structure preserved
 ...
 */
static pub unsafe fn setappname(char *title);
static pub unsafe fn getappname(HANDLE Instance);

pub unsafe fn initapp(pub unsafe fn argc, char **argv);
pub unsafe fn exitapp(void);
pub unsafe fn gabeep(void);

/*
 *  Initialise application. Returns zero on failure.
 */
pub unsafe fn initapp(pub unsafe fn argc, char **argv) -> c_int {
{
    if (! app_initialised)
    {
	app_initialised = 1;
 ...
 */
PROTECTED
pub unsafe fn app_cleanup(void) {
{
    if (app_initialised) {
	app_initialised = 0;
	finish_contexts();
 ...
 */
pub unsafe fn execapp(char *app) -> c_int {
{
#ifdef __MSDOS__
...
pub unsafe fn startgraphapp(HINSTANCE Instance, HINSTANCE PrevInstance, int CmdShow) {
{
    /*
