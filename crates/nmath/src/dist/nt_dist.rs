...
pub fn pnt_inner(t: f64, df: f64, ncp: f64, lower_tail: bool, log_p: bool) -> f64 {
    ...
    if x > 0.0 {
        ...
        if p == 0.0 {
            ...
            crate::error::ml_warning(crate::constants::ME_UNDERFLOW, "pnt");
            crate::error::ml_warning(crate::constants::ME_RANGE, "pnt");
            return r_dt_0(lower_tail, log_p);
        }
        q = M_SQRT_2dPI * p * del;
        s = 0.5 - p;
    ...
    if tnc > 1.0 - 1e-10 && lower_tail {
        crate::error::ml_warning(crate::constants::ME_PRECISION, "pnt{final}");
    }
...
#[must_use]
pub fn Rf_dnt(x: c_double, df: c_double, ncp: c_double, give_log: c_int) -> c_double {
    dnt_inner(x, df, ncp, give_log != 0)
}

#[must_use]
pub fn dnt(x: c_double, df: c_double, ncp: c_double, give_log: c_int) -> c_double {
    dnt_inner(x, df, ncp, give_log != 0)
}
...
#[must_use]
pub fn Rf_rnt(df: c_double, ncp: c_double) -> c_double {
    rnt_inner(df, ncp)
}

#[must_use]
pub fn rnt(df: c_double, ncp: c_double) -> c_double {
    rnt_inner(df, ncp)
}
