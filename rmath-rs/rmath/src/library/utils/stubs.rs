...
pub unsafe fn fileedit(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    ...
    if n > 0 {
        ...
        for i in 0..n {
            ...
            if elt.is_null() || elt == R_NilValue() {
                let msg = CString::new("'filename' contains missing values").unwrap_or_default();
                Rf_error(msg.as_ptr());
            }
            let c = CHAR(elt);
            if !c.is_null() {
    ...
    if let Err(e) = result {
        let msg =
            CString::new(format!("unable to run editor '{}': {}", editor, e)).unwrap_or_default();
        Rf_warning(msg.as_ptr());
    }
...
pub unsafe fn charClass(x: SEXP, scl: SEXP) -> SEXP {
    ...
    if !isString(scl) || LENGTH(scl) != 1 {
        let msg = CString::new("argument 'class' must be a character string").unwrap_or_default();
        Rf_error(msg.as_ptr());
    }
    ...
    let wcl = crate::main::rlocale::Ri18n_wctype(cl_cstr.as_ptr() as *const std::os::raw::c_uchar);
    if wcl == 0 {
        let msg =
...
pub unsafe fn nsl(hostname: SEXP) -> SEXP {
    if !isString(hostname) || LENGTH(hostname) != 1 {
        let msg =
            CString::new("'hostname' must be a character vector of length 1").unwrap_or_default();
        Rf_error(msg.as_ptr());
    }
    ...
    if !found {
        let msg = CString::new("unknown format returned by name resolution").unwrap_or_default();
        Rf_warning(msg.as_ptr());
    }
