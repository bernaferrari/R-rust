...
pub unsafe fn isNAcol(col: SEXP, index: c_int, ncol: c_int) -> Rboolean {
    ...
    if Rf_isNull(col) != 0 {
        ...
        if isLogical(col) != 0 {
            result = (LOGICAL(col).add((index % ncol) as usize).read() == NA_LOGICAL) as Rboolean;
            ...
            result = (INTEGER(col).add((index % ncol) as usize).read() == NA_INTEGER) as Rboolean;
    ...
    if isList(pch) != 0 {
        ...
        for i in 0..n as usize {
            if LOGICAL(pch).add(i).read() == NA_LOGICAL {
                ...
                Rf_error(b"only NA allowed in logical plotting symbol\0".as_ptr() as *const c_char);
            }
        }
    } else {
            ...
            if nms == R_NilValue() {
                ...
                for i in 0..n as usize {
                    ...
                    if strcmp(nm_str, b"cex\0".as_ptr() as *const c_char) == 0 {
                        ...
                        font = asInteger(FixupFont(VECTOR_ELT(spec, i as R_xlen_t), NA_INTEGER));
...
unsafe fn xypoints(args: SEXP, n: *mut c_int) {
    ...
    *n = k;
    ...
    if *n > 0 && kmin == 0 {
        Rf_error(
            b"cannot mix zero-length and non-zero-length coordinates\0".as_ptr() as *const c_char,
        );
    }
}
