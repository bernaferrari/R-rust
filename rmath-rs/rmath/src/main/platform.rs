...
pub(crate) unsafe fn R_strieql(a: *const c_char, b: *const c_char) -> c_int {
    unsafe {
...
pub unsafe extern "C" fn do_fileinfo(_call: SEXP, _op: SEXP, args: SEXP, _rho: SEXP) -> SEXP {
    unsafe {
        ...
        crate::attrib_core::setAttrib(ans, crate::attrib_core::R_NamesSymbol(), rn);
        ...
        crate::attrib_core::setAttrib(
            crate::attrib_core::getAttrib(ans, crate::attrib_core::R_NamesSymbol()),
...
pub unsafe extern "C" fn do_localeconv(_call: SEXP, _op: SEXP, _args: SEXP, _rho: SEXP) -> SEXP {
    unsafe {
        ...
        crate::attrib_core::setAttrib(ans, crate::attrib_core::R_NamesSymbol(), names);
        Rf_unprotect(2);
        ans
    }
}
...
// ---------------------------------------------------------------------------
// do_getwd — get working directory
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_getwd(_call: SEXP, _op: SEXP, _args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        match std::env::current_dir() {
            ...
            Err(_) => {
                ...
                use crate::sexp::ffi::SEXPTYPE;
                let s = Rf_allocVector(SEXPTYPE::STRSXP.0, 1);
                SET_STRING_ELT(s, 0, Rf_mkChar(b"\0".as_ptr() as *const c_char));
                s
            }
        }
    }
...
// ---------------------------------------------------------------------------
// do_setwd — set working directory
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_setwd(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::sexp::accessors::*;
        ...
        let path_cstr = std::ffi::CStr::from_ptr(CHAR(STRING_ELT(path_arg, 0)));
...
// ---------------------------------------------------------------------------
// do_basename — extract file basename
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_basename(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        ...
        let len = LENGTH(x);
        let s = Rf_protect(Rf_allocVector(SEXPTYPE::STRSXP.0, len));
        let na = NA_STRING();

        for i in 0..len as usize {
            let el = STRING_ELT(x, i as i64);
            if el == na || el.is_null() {
                SET_STRING_ELT(s, i as i64, na);
            } else {
                let cs = std::ffi::CStr::from_ptr(CHAR(el)).to_str().unwrap_or("");
                let base = std::path::Path::new(cs)
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("");
...
// ---------------------------------------------------------------------------
// do_dirname — extract directory name
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_dirname(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::main::relop::NA_STRING;
        use crate::sexp::accessors::*;
        use crate::sexp::constructors::*;
...
// ---------------------------------------------------------------------------
// do_Sys_sleep — sleep for given number of seconds
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_Sys_sleep(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::sexp::accessors::*;
        use crate::sexp::ffi::SEXPTYPE;
        use crate::sexp::globals::R_NilValue;
        ...
        let secs = if TYPEOF(time_arg) == SEXPTYPE::REALSXP.0 {
            *crate::sexp::accessors::REAL(time_arg)
        } else {
            crate::main::coerce::asInteger(time_arg) as f64
        };
...
// ---------------------------------------------------------------------------
// do_gc — garbage collection (stub)
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_gc(_call: SEXP, _op: SEXP, _args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::sexp::constructors::*;
        use crate::sexp::ffi::SEXPTYPE;
        use crate::sexp::protect::{Rf_protect, Rf_unprotect};

        // Return named numeric vector with gc stats
        // (real GC would populate these)
        let n = 6i32;
        let s = Rf_protect(Rf_allocVector(SEXPTYPE::REALSXP.0, n));
        for i in 0..n as usize {
            *crate::sexp::accessors::REAL(s).add(i) = 0.0;
        }
        Rf_unprotect(1);
        s
    }
...
// ---------------------------------------------------------------------------
// do_gcinfo — report gc info
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_gcinfo(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::sexp::constructors::Rf_ScalarLogical;
...
// ---------------------------------------------------------------------------
// do_proc_time — process time
// ---------------------------------------------------------------------------

#[unsafe(no_mangle)]
pub unsafe extern "C" fn do_proc_time(_call: SEXP, _op: SEXP, _args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        use crate::sexp::accessors::*;
        use crate::sexp::constructors::*;
        use crate::sexp::ffi::SEXPTYPE;
        ...
        let s = Rf_protect(Rf_allocVector(SEXPTYPE::REALSXP.0, 5));
        let p = REAL(s);
        // user.self, sys.self, user.children, sys.children, elapsed
        *p.add(0) = 0.0;
        *p.add(1) = 0.0;
        *p.add(2) = 0.0;
        *p.add(3) = 0.0;
        *p.add(4) = 0.0;
        ...
        SET_STRING_ELT(
            names,
            0,
            Rf_mkChar(b"user.self\0".as_ptr() as *const c_char),
        );
        SET_STRING_ELT(names, 1, Rf_mkChar(b"sys.self\0".as_ptr() as *const c_char));
        SET_STRING_ELT(
            names,
            2,
            Rf_mkChar(b"user.children\0".as_ptr() as *const c_char),
        );
        SET_STRING_ELT(
            names,
            3,
            Rf_mkChar(b"sys.children\0".as_ptr() as *const c_char),
        );
        SET_STRING_ELT(names, 4, Rf_mkChar(b"elapsed\0".as_ptr() as *const c_char));
        crate::attrib_core::setAttrib(s, crate::attrib_core::R_NamesSymbol(), names);
