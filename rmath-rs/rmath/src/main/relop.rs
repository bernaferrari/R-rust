...
use crate::main::identical::R_compute_identical;
...
/// errorcall -- delegates to the real error infrastructure.
pub unsafe fn errorcall_stub(call: SEXP, format: *const c_char) {
    crate::main::errors::errorcall(call, format);
}

/// error -- delegates to the real error infrastructure.
pub unsafe fn error_stub(format: *const c_char) {
    crate::main::errors::Rf_error(format);
}

/// warningcall -- delegates to the real warning infrastructure.
...
pub unsafe fn isTs(x: SEXP) -> c_int {
    unsafe {
        ...
        let tsp = crate::attrib_core::getAttrib(x, crate::attrib_core::R_TspSymbol());
...
pub unsafe fn do_relop(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        if ATTRIB(arg1) != R_NilValue() || ATTRIB(arg2) != R_NilValue() {
            if crate::main::objects::DispatchGroup(
                arg1,
