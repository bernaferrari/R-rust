...
/// SEXPTYPE integer values for match patterns -- now imported from crate::sexp::ffi::SEXPTYPE.
...
unsafe fn IS_BYTES(s: SEXP) -> bool {
    unsafe {
        if s.is_null() {
            return false;
        }
        let gp = (*s).sxpinfo.gp();
        (gp & 0x02) != 0 // GP_BYTES_MASK = 1 << 1
    }
}

unsafe fn IS_LATIN1(s: SEXP) -> bool {
    unsafe {
        if s.is_null() {
            return false;
        }
        let gp = (*s).sxpinfo.gp();
        (gp & 0x04) != 0 // GP_LATIN1_MASK = 1 << 2
    }
}
...
unsafe fn ENC_KNOWN(s: SEXP) -> c_int {
    unsafe {
        ...
        let gp = (*s).sxpinfo.gp();
        if (gp & 0x08) != 0 || (gp & 0x04) != 0 || (gp & 0x02) != 0 {
            1
        } else {
            0
        }
...
unsafe fn asBool2(x: SEXP, call: SEXP) -> bool {
    unsafe { crate::main::coerce::asRbool(x, call) != 0 }
}
...
unsafe fn PrintDefaults() {
    unsafe {
        crate::main::print::PrintDefaults();
    }
}

unsafe fn EncodeLogical(x: c_int, w: c_int) -> *const c_char {
    unsafe { crate::main::printutils::EncodeLogical(x, w) }
}
...
unsafe fn EncodeReal0(
    ...
    unsafe { crate::main::printutils::EncodeReal0(x, w, d, e, outdec) }
}
...
unsafe fn formatLogical(x: *const c_int, n: R_xlen_t, fieldwidth: *mut c_int) {
    unsafe {
        crate::main::format::formatLogical(x, n, fieldwidth);
    }
}

unsafe fn formatInteger(x: *const c_int, n: R_xlen_t, fieldwidth: *mut c_int) {
    unsafe {
        crate::main::format::formatInteger(x, n, fieldwidth);
    }
}
...
unsafe fn formatReal(
    ...
    unsafe {
        ...
        let mut dr: c_int = 0;
        let mut er: c_int = 0;
        crate::main::format::formatReal(x, n, &mut wr, &mut dr, &mut er, nsmall);
        if !w.is_null() {
            *w = wr;
        }
        if !d.is_null() {
...
unsafe fn formatRaw(x: *const crate::sexp::ffi::Rbyte, n: R_xlen_t, fieldwidth: *mut c_int) {
    unsafe {
        crate::main::format::formatRaw(x as *const std::ffi::c_void, n, fieldwidth);
    }
}
...
unsafe fn isObject(x: SEXP) -> c_int {
    unsafe { crate::attrib_core::isObject(x) }
}
...
unsafe fn duplicate(s: SEXP) -> SEXP {
    unsafe { crate::main::duplicate::duplicate(s) }
}
...
pub unsafe fn do_format(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        } else if TYPEOF(x) == SEXPTYPE::EXTPTRSXP.0 {
