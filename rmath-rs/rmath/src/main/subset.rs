...
use crate::main::subscript::{get1index, int_arraySubscript, makeSubscript};
...
// ---------------------------------------------------------------------------
// Ported from R's DropDims() in src/main/array.c.

/// Drop dimensions of length 1 from result.
/// Mutates x in place (caller should duplicate first).
unsafe fn DropDims(x: SEXP) -> SEXP {
    unsafe {
        ...
        let dim = INTEGER(dims);

        // (2) Check whether there are redundant extents
        let mut n: c_int = 0;
        for i in 0..ndims {
            if *dim.add(i as usize) != 1 {
                n += 1;
            }
        }
        if n == ndims {
        ...
        let mut dimnames = Rf_protect(getAttrib(x, sym_DimNames()));
        let mut newnames: SEXP = R_NilValue();

        if n <= 1 {
            // Reduced to a vector result.
            if !isNull(dimnames) {
                if XLENGTH(x) != 1 {
                    for i in 0..ndims {
                        if *dim.add(i as usize) != 1 {
                            newnames = VECTOR_ELT(dimnames, i as R_xlen_t);
                            break;
                        }
                    }
                } else {
                    ...
                    let mut cnt: c_int = 0;
                    for i in 0..ndims {
                        if !isNull(VECTOR_ELT(dimnames, i as R_xlen_t)) {
                            cnt += 1;
                        }
                    }
                    if cnt == 1 {
            ...
            setAttrib(x, sym_Names(), newnames);
            Rf_unprotect(1); // newnames
        } else {
            // Lower dimensional array
            let dnn = Rf_protect(getAttrib(dimnames, sym_Names()));
            let newdims = Rf_protect(Rf_allocVector(SEXPTYPE::INTSXP.0, n));
            let mut j: c_int = 0;
            for i in 0..ndims {
                if *dim.add(i as usize) != 1 {
                    *INTEGER(newdims).add(j as usize) = *dim.add(i as usize);
                    j += 1;
                }
            }
            if !isNull(getAttrib(dims, sym_Names())) {
                let new_nms = Rf_protect(Rf_allocVector(SEXPTYPE::STRSXP.0, n));
                let nms_d = getAttrib(dims, sym_Names());
                j = 0;
                for i in 0..ndims {
                    if *dim.add(i as usize) != 1 {
                        SET_STRING_ELT(new_nms, j as R_xlen_t, STRING_ELT(nms_d, i as R_xlen_t));
                        j += 1;
                    }
                }
                setAttrib(newdims, sym_Names(), new_nms);
                Rf_unprotect(1); // new_nms
            }
            let mut havenames = false;
            if !isNull(dimnames) {
                for i in 0..ndims {
                    if *dim.add(i as usize) != 1 && !isNull(VECTOR_ELT(dimnames, i as R_xlen_t)) {
                        havenames = true;
                        break;
                    }
                }
                if havenames {
                    newnames = Rf_protect(Rf_allocVector(SEXPTYPE::VECSXP.0, n));
                    let newnamesnames = Rf_protect(Rf_allocVector(SEXPTYPE::STRSXP.0, n));
                    j = 0;
                    for i in 0..ndims {
                        if *dim.add(i as usize) != 1 {
                            if !isNull(dnn) {
                    ...
                    if !isNull(dnn) {
                        setAttrib(newnames, sym_Names(), newnamesnames);
                    }
                    setAttrib(x, sym_DimNames(), newnames);
                    Rf_unprotect(2); // newnamesnames, newnames
                } else {
                    dimnames = R_NilValue();
                }
            }
            setAttrib(x, sym_DimNames(), R_NilValue());
            setAttrib(x, sym_Dim(), newdims);
            if havenames {
                Rf_unprotect(2); // newdims, dnn
            } else {
                Rf_unprotect(2); // newdims, dnn
            }
        }
        Rf_unprotect(2); // dimnames, x
        x
    }
}
        ...
        if nsubs < 2 {
            ...
            if ndim == 1 {
                ...
                if drop == 0 || len > 1 {
                    ...
                    if !isNull(attrib) {
                        let nattrib = Rf_protect(crate::main::duplicate::duplicate(attrib));
...
pub unsafe extern "C" fn fixSubset3Args(
    ...
    unsafe {
        ...
        let new_args = crate::main::duplicate::shallow_duplicate(args);
...
pub unsafe extern "C" fn do_subassign(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        crate::main::subassign::do_subassign_dflt(call, op, args, env)
    }
}
...
pub unsafe extern "C" fn do_subassign2(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        /* Fall through to default -- delegated to subassign module */
        crate::main::subassign::do_subassign2_dflt(call, op, args, env)
    }
}
