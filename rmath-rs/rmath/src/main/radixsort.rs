    ...
    unsafe {
        ...
        match xtype {
            ...
            t if t == SEXPTYPE::STRSXP.0 => {
                // TODO: implement csorted — currently returns 0 (not sorted)
                tmp = csorted(xd, n);
            }
            _ => {
                std::panic::panic_any(crate::sexp::context::RError {
                    message: format!("First arg is type '{}', not supported by radix sort", xtype),
                });
            }
        }
        ...
        while col <= narg {
            ...
            let xtype = TYPEOF(x);
            // Handle INTSXP/LGLSXP/REALSXP for multi-column
            if xtype != SEXPTYPE::INTSXP.0
                && xtype != SEXPTYPE::LGLSXP.0
                && xtype != SEXPTYPE::REALSXP.0
            {
            ...
            let mut grp: c_int = 0;
            while grp < ngrp {
                let thisgrpn: c_int = GS[(1 - FLIP) as usize].add(grp as usize).read();
                if thisgrpn == 1 {
                    ...
                    if NALAST == 0 {
                        if *o.add(idx as usize) == 0 {
                            ...
                            let v = *REAL(x).add(*o.add(idx as usize) as usize - 1);
                            if v.is_nan() {
                                isSorted = false;
                                *o.add(idx as usize) = 0;
                            }
                ...
                if xtype == SEXPTYPE::REALSXP.0 {
                    for j in 0..thisgrpn as usize {
                        *(xsub as *mut f64).add(j) = *(xd_col as *const f64)
...
pub unsafe extern "C" fn csort(x: *mut c_void, o: *mut c_int, n: c_int) -> *mut c_void {
    unsafe {
        ...
        // Helper: compare two string indices for sorting
        let cmp_indices = |a: c_int, b: c_int| -> i32 {
            let sa = *xs.add((a - 1) as usize);
            let sb = *xs.add((b - 1) as usize);
            // NA handling
            if sa.is_null() && sb.is_null() {
                return 0;
            }
            if sa.is_null() {
                return if NALAST == 1 { 1 } else { -1 };
            }
            if sb.is_null() {
                return if NALAST == 1 { -1 } else { 1 };
            }
            ...
            let ca = std::ffi::CStr::from_ptr(pa);
            let cb = std::ffi::CStr::from_ptr(pb);
            match ca.cmp(cb) {
        ...
        // Initialize o with 1..n
        for i in 0..n as usize {
            *o.add(i) = (i + 1) as c_int;
        }

        // Insertion sort for small n
        if n < N_SMALL as c_int {
            for i in 1..n as usize {
                let key = *o.add(i);
                let mut j = i as isize - 1;
                while j >= 0 {
                    let cmp = cmp_indices(*o.add(j as usize), key);
                    let ordered_cmp = if ORDER == -1 { -cmp } else { cmp };
                    if ordered_cmp > 0 {
                        *o.add((j + 1) as usize) = *o.add(j as usize);
                        j -= 1;
                    } else {
                ...
                *o.add((j + 1) as usize) = key;
            }
        } else {
            // Use a stable sort via Vec
            let mut indices: Vec<c_int> = Vec::with_capacity(n as usize);
            for i in 0..n as usize {
                indices.push((i + 1) as c_int);
            }
            let order_val = ORDER;
            indices.sort_by(|&a, &b| {
                let cmp = cmp_indices(a, b);
                (cmp * order_val).cmp(&0)
            });
            for i in 0..n as usize {
                *o.add(i) = indices[i];
        ...
        // Push group sizes if needed
        if STACKGRPS {
            let mut tt: c_int = 1;
            for i in 1..n as usize {
                let cmp = cmp_indices(*o.add(i), *o.add(i - 1));
                let ordered_cmp = if ORDER == -1 { -cmp } else { cmp };
                if ordered_cmp == 0 {
                    tt += 1;
                } else {
...
/// Grouping for character data — find group boundaries without sorting.
///
...
pub unsafe extern "C" fn cgroup(x: *mut c_void, o: *mut c_int, n: c_int) -> *mut c_void {
    unsafe {
        ...
        // Build a mapping from CHARSXP to first-appearance index
        // Use a simple Vec as a hash set of seen string pointers
        let mut seen: Vec<SEXP> = Vec::new();
        let mut first_idx: Vec<c_int> = Vec::new();
        ...
        for i in 0..n as usize {
            ...
            if !found {
                seen.push(s);
                first_idx.push(i as c_int);
            }
        }

        // Count occurrences of each unique string
        let mut counts: Vec<c_int> = vec![0; seen.len()];
        for i in 0..n as usize {
            let s = *xs.add(i);
            for j in 0..seen.len() {
                if seen[j] == s {
        ...
        // Build order: group by first-appearance order
        let target = if *o != -1 { NEWO } else { o };
        let mut pos: Vec<c_int> = vec![0; seen.len()];
        for i in 0..n as usize {
            let s = *xs.add(i);
            for j in 0..seen.len() {
                if seen[j] == s {
                    let k = pos[j] + first_idx[j];
                    if !target.is_null() {
                        *target.add(k as usize) = (i + 1) as c_int;
                    }
                    pos[j] += 1;
                    break;
                }
...
///
/// `x` is an array of CHARSXP pointers (DATAPTR of a STRSXP).
/// Compares strings via CHAR() using C string comparison.
///
/// Returns:
/// - `1` if sorted in expected ORDER direction
...
pub unsafe extern "C" fn csorted(x: *mut c_void, n: c_int) -> c_int {
    unsafe {
        ...
        // Helper: compare two CHARSXP, respecting ORDER
        let cmp_str = |a: SEXP, b: SEXP| -> i32 {
            if a.is_null() && b.is_null() {
                return 0;
            }
            if a.is_null() {
                // NA sorts based on NALAST
                return if NALAST == 1 { 1 } else { -1 };
            }
            if b.is_null() {
                return if NALAST == 1 { -1 } else { 1 };
        ...
        let v0 = *xs.add(0);
        let v1 = *xs.add(1);
        let cmp01 = cmp_str(v1, v0);

        if cmp01 < 0 {
            // Check if strictly opposite
            let mut all_opp = true;
            let mut i: c_int = 2;
            while i < n {
                if cmp_str(*xs.add(i as usize), *xs.add((i - 1) as usize)) >= 0 {
                    all_opp = false;
                    break;
                }
                i += 1;
        ...
        let mut tt: c_int = 1;
        for i in 1..n as usize {
            let tmp = cmp_str(*xs.add(i), *xs.add(i - 1));
            if tmp < 0 {
                GSNGRP[FLIP as usize] = old;
                return 0;
            }
