        ...
        if value_opt {
            ...
            for i in 0..n {
                ...
                if m {
                    ...
                    let empty_str = std::ffi::CString::new("").expect("CString::new failed: contains null byte");
...
pub unsafe fn do_regexpr(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        let match_len_str = std::ffi::CString::new("match.length").expect("CString::new failed: contains null byte");
...
// ---------------------------------------------------------------------------
// do_grepl -- grep with logical (TRUE/FALSE) return
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// do_sub -- sub() replacement (first match only)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// do_gregexpr -- find all matches of pattern in each string
// ---------------------------------------------------------------------------
...
pub unsafe fn do_gregexpr(call: SEXP, op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        // Allocate list result
        let ans = Rf_protect(allocVector(SEXPTYPE::VECSXP.0, n));
        let matchlen_list = Rf_protect(allocVector(SEXPTYPE::VECSXP.0, n));

        for i in 0..n {
            let text_charsxp = STRING_ELT(text, i as R_xlen_t);
            if isNA_STRING(text_charsxp) {
                let el = allocVector(SEXPTYPE::INTSXP.0, 1);
                *INTEGER(el) = NA_INTEGER;
                SET_VECTOR_ELT(ans, i as R_xlen_t, el);
                let ml = allocVector(SEXPTYPE::INTSXP.0, 1);
                *INTEGER(ml) = NA_INTEGER;
                SET_VECTOR_ELT(matchlen_list, i as R_xlen_t, ml);
                continue;
            }

            let text_str = cstr_to_string(CHAR(text_charsxp));
            let mut positions: Vec<c_int> = Vec::new();
            let mut lengths: Vec<c_int> = Vec::new();
            ...
            loop {
                ...
                let result = if fixed_opt {
                    let pos =
                        R_grep_fixed_inner(pat_str.as_bytes(), remaining.as_bytes(), igcase_opt);
                    if pos >= 0 {
                        Some(EreMatch {
                            start: pos as usize,
                            end: pos as usize + pat_str.len(),
                    ...
                    ere_search(nodes, remaining.as_bytes(), igcase_opt)
                } else {
                    None
                };

                if let Some(m) = result {
                    positions.push((search_start + m.start + 1) as c_int); // 1-based
                    lengths.push((m.end - m.start) as c_int);
                    let new_start = search_start + m.end;
                    if m.end == 0 {
                        search_start += 1;
                    } else {
            ...
            if positions.is_empty() {
                ...
                *INTEGER(ml) = -1;
                ...
                let el = allocVector(SEXPTYPE::INTSXP.0, positions.len() as R_xlen_t);
                let ml = allocVector(SEXPTYPE::INTSXP.0, lengths.len() as R_xlen_t);
                for (j, &pos) in positions.iter().enumerate() {
                    *INTEGER(el).add(j) = pos;
                }
                for (j, &len) in lengths.iter().enumerate() {
                    *INTEGER(ml).add(j) = len;
                }
                SET_VECTOR_ELT(ans, i as R_xlen_t, el);
                SET_VECTOR_ELT(matchlen_list, i as R_xlen_t, ml);
            }
        ...
        let match_len_str = std::ffi::CString::new("match.length").expect("CString::new failed: contains null byte");
        let names_sym = Rf_mkChar(match_len_str.as_ptr());
        setAttrib(ans as SEXP, names_sym, matchlen_list);
...
// ---------------------------------------------------------------------------
// do_pcre_config -- PCRE configuration query
// ---------------------------------------------------------------------------
