...
//!
//! This module ports the complex number operations from R's complex.c.
//! The polyroot functions are now in `polyroot.rs`.
//!
...
/// Convert an Rcomplex to a num::Complex64.
#[inline]
fn to_complex(x: &Rcomplex) -> Complex64 {
    Complex64::new(x.r, x.i)
}

/// Convert a num::Complex64 to an Rcomplex.
#[inline]
fn from_complex(z: Complex64) -> Rcomplex {
...
pub fn R_cpow_n(x: Complex64, k: i32) -> Complex64 {
    if k == 0 {
...
pub fn mycpow(x: Complex64, y: Complex64) -> Complex64 {
    let yr = y.re;
    let yi = y.im;

    if x == Complex64::new(0.0, 0.0) {
        if yi == 0.0 {
            Complex64::new(R_pow(0.0, yr), 0.0)
        } else {
...
pub fn clog(x: Complex64) -> Complex64 {
    let xr = x.re;
    let xi = x.im;
    Complex64::new(xr.hypot(xi).ln(), xi.atan2(xr))
}

/// Complex square root via mycpow(z, 0.5).
///
/// Ported from lines 407-414 of complex.c.
pub fn csqrt(x: Complex64) -> Complex64 {
    mycpow(x, Complex64::new(0.5, 0.0))
}
...
pub fn catan(z: Complex64) -> Complex64 {
    let x = z.re;
    let y = z.im;
...
pub fn ccosh(z: Complex64) -> Complex64 {
    ccos(z * Complex64::new(0.0, 1.0))
}

/// Complex hyperbolic sine: -i*sin(z*i) (A&S 4.5.7).
///
/// Ported from lines 507-513 of complex.c.
pub fn csinh(z: Complex64) -> Complex64 {
    let result = csin(z * Complex64::new(0.0, 1.0));
    Complex64::new(result.im, -result.re)
}
...
pub fn z_tan(z: Complex64) -> Complex64 {
    let y = z.im;
    let mut r = ctan(z);
    if R_FINITE(y) && y.abs() > 25.0 {
        ...
        r = Complex64::new(r.re, if y < 0.0 { -1.0 } else { 1.0 });
    }
    r
}
...
pub fn ctanh(z: Complex64) -> Complex64 {
    let result = z_tan(z * Complex64::new(0.0, 1.0));
    Complex64::new(result.im, -result.re)
}
...
pub fn z_asin(z: Complex64) -> Complex64 {
...
pub fn z_acos(z: Complex64) -> Complex64 {
    if z.im == 0.0 && z.re.abs() > 1.0 {
...
pub fn z_atan(z: Complex64) -> Complex64 {
...
pub fn z_acosh(z: Complex64) -> Complex64 {
    let result = z_acos(z);
    Complex64::new(-result.im, result.re)
}
...
pub fn z_asinh(z: Complex64) -> Complex64 {
    let result = z_asin(z * Complex64::new(0.0, 1.0));
    Complex64::new(result.im, -result.re)
}
...
pub fn z_atanh(z: Complex64) -> Complex64 {
    let result = z_atan(z * Complex64::new(0.0, 1.0));
    Complex64::new(result.im, -result.re)
}
...
pub fn z_atan2(r: &mut Rcomplex, csn: &Rcomplex, ccs: &Rcomplex) {
    ...
    if dccs == Complex64::new(0.0, 0.0) {
        if dcsn == Complex64::new(0.0, 0.0) {
            r.r = NA_REAL;
            r.i = NA_REAL;
            return;
        }
        ...
        *r = from_complex(Complex64::new(real_part, 0.0));
        return;
    }
...
mod tests {
    ...
    #[test]
    fn test_csin_real() {
        let z = Complex64::new(std::f64::consts::FRAC_PI_2, 0.0);
        let result = csin(z);
        assert!((result.re - 1.0).abs() < 1e-10);
        assert!(result.im.abs() < 1e-10);
    }
    ...
    fn test_ctan_pi_over_4() {
        let z = Complex64::new(std::f64::consts::FRAC_PI_4, 0.0);
        let result = ctan(z);
        assert!((result.re - 1.0).abs() < 1e-10);
        assert!(result.im.abs() < 1e-10);
    }
