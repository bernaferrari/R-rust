...
use crate::attrib_core::{R_ClassSymbol, R_data_class, getAttrib, isObject, setAttrib};
...
pub unsafe fn R_LookupMethod(
    ...
    unsafe {
        ...
        // Validate callrho — must be a valid environment or null
        if !callrho.is_null()
            && callrho != R_NilValue()
            && TYPEOF(callrho) != SEXPTYPE::ENVSXP.0 as c_int
        {
            std::panic::panic_any(crate::sexp::context::RError {
                message: "bad generic call environment".to_string(),
            });
        }
        if callrho.is_null() || callrho == R_NilValue() {
            return R_NilValue();
        }
...
pub unsafe fn usemethod(
    ...
    unsafe {
        ...
        for i in 0..nclass {
            ...
            let method = crate::main::names::installS3Signature(generic, ss);
        ...
        // Try default method
        let default_method = crate::main::names::installS3Signature(
            generic,
...
pub unsafe fn do_nextmethod(call: SEXP, _op: SEXP, args: SEXP, env: SEXP) -> SEXP {
    unsafe {
        ...
        for ii in j..nclass {
            ...
            nextfunSignature = crate::main::names::installS3Signature(sg, sk);
            ...
            if group_val != R_UnboundValue() {
                ...
                nextfunSignature = crate::main::names::installS3Signature(sb2, sk);
                nextfun = R_LookupMethod(nextfunSignature, env, callenv, defenv);
                if isFunction(nextfun) != FALSE {
                    i = ii;
                    break;
        ...
        if isFunction(nextfun) == FALSE {
            // Try default method
            nextfunSignature = crate::main::names::installS3Signature(
                sg,
                b"default\x00".as_ptr() as *const c_char,
            );
            nextfun = R_LookupMethod(nextfunSignature, env, callenv, defenv);
...
pub unsafe fn R_set_standardGeneric_ptr(
    ...
    unsafe {
        let old = R_STANDARD_GENERIC_PTR.with(|v| {
            let old = v.get();
            v.set(val);
            old
        });
...
pub unsafe fn do_set_prim_method(
    ...
    unsafe {
        ...
        if PRIM_METHODS.with(|v| v.get()).is_null() {
            ...
            PRIM_METHODS.with(|v| v.set(libc::malloc(std::mem::size_of::<prim_methods_t>() * n) as *mut prim_methods_t));
            PRIM_GENERICS
...
pub unsafe fn findmethod(
    ...
    unsafe {
        ...
        for i in 0..nclass {
            let ss = translateChar(STRING_ELT(klass, i as R_xlen_t));
            let m = crate::main::names::installS3Signature(generic, ss);
        ...
        // Try default
        let m = crate::main::names::installS3Signature(
            generic,
            b"default\x00".as_ptr() as *const c_char,
        );
        let sxp = R_LookupMethod(m, ptr::null_mut(), ptr::null_mut(), ptr::null_mut());
...
pub(crate) unsafe fn DispatchOrEval_objects(
    ...
    unsafe {
        ...
        for i in 0..nclass {
            let ss = translateChar(STRING_ELT(klass, i as R_xlen_t));
            let method_sym = crate::main::names::installS3Signature(generic, ss);
