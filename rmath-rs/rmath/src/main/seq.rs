...
use crate::sexp::accessors::{
    CADDDR, CADDR, CADR, CAR, CDDDR, CDDR, CDR, CHAR, COMPLEX, INTEGER, LENGTH, LOGICAL, OBJECT,
    RAW, REAL, SET_STRING_ELT, SET_VECTOR_ELT, SETCAR, SETTAG, STRING_ELT, TYPEOF, VECTOR_ELT,
    XLENGTH,
};
use crate::sexp::constructors::{
    Rf_ScalarInteger, Rf_ScalarReal, Rf_allocVector, Rf_isInteger, Rf_isNull, Rf_isVector,
...
// ---------------------------------------------------------------------------
// Local wrappers that delegate to real implementations
...
unsafe fn check1arg(args: SEXP, call: SEXP, name: *const c_char) {
    crate::main::errors::check1arg(args, call, name);
}

unsafe fn errorcall(call: SEXP, format: *const c_char) {
    crate::main::errors::errorcall(call, format);
}

unsafe fn warningcall(call: SEXP, format: *const c_char) {
    crate::main::errors::warningcall(call, format);
}

unsafe fn R_typeToChar(s: SEXP) -> *const c_char {
    unsafe { crate::main::util_main::type2char(TYPEOF(s)) }
}
...
unsafe fn UNIMPLEMENTED_TYPE(mesg: *const c_char, s: SEXP) {
    unsafe {
        crate::main::relop::UNIMPLEMENTED_TYPE(mesg, s);
    }
}
...
/// Check if object inherits from the given class name.
unsafe fn inherits(s: SEXP, what: *const c_char) -> c_int {
    unsafe {
        if s.is_null() || what.is_null() {
            return 0;
        }
        let klass = crate::attrib_core::getAttrib(s, crate::attrib_core::R_ClassSymbol());
        if klass == R_NilValue() || TYPEOF(klass) != SEXPTYPE::STRSXP.0 {
            return 0;
        }
        let cn = match std::ffi::CStr::from_ptr(what).to_str() {
            Ok(s) => s,
            Err(_) => return 0,
        };
        let n = LENGTH(klass);
        for i in 0..n {
            ...
            if !elt.is_null() {
                let cs = CHAR(elt);
                if !cs.is_null() {
                    let s2 = std::ffi::CStr::from_ptr(cs).to_str().unwrap_or("");
                    if s2 == cn {
                        return 1;
                    }
                }
...
unsafe fn lazy_duplicate(x: SEXP) -> SEXP {
    unsafe { crate::main::duplicate::lazy_duplicate(x) }
}

unsafe fn Rf_duplicate(x: SEXP) -> SEXP {
    unsafe { crate::main::duplicate::Rf_duplicate(x) }
}
...
unsafe fn R_LevelsSymbol() -> SEXP {
    unsafe { crate::attrib_core::R_LevelsSymbol() }
}
...
unsafe fn asBool2(x: SEXP, call: SEXP) -> c_int {
    unsafe { crate::main::coerce::asRbool(x, call) }
}
...
unsafe fn isReal(x: SEXP) -> c_int {
    unsafe { if crate::main::coerce::isReal(x) { 1 } else { 0 } }
}
...
unsafe fn R_DotsSymbol() -> SEXP {
    unsafe { crate::sexp::symbol::Rf_install(c"...".as_ptr()) }
}
...
unsafe fn cross_colon(call: SEXP, s: SEXP, t: SEXP) -> SEXP {
    unsafe {
        ...
        let nls = LENGTH(ls);
        let nlt = LENGTH(lt);
        let a = Rf_allocVector(SEXPTYPE::INTSXP.0, n);
        let rs = coerceVector(s, SEXPTYPE::INTSXP.0);
        let rt = coerceVector(t, SEXPTYPE::INTSXP.0);
        ...
        if Rf_isNull(ls) == 0 && Rf_isNull(lt) == 0 {
            let la = Rf_allocVector(
                SEXPTYPE::STRSXP.0,
                (nls as R_xlen_t * nlt as R_xlen_t) as c_int,
            );
...
unsafe fn seq_colon(n1: c_double, n2: c_double, call: SEXP) -> SEXP {
    unsafe {
        ...
        if use_int {
            ...
            let ans = Rf_allocVector(SEXPTYPE::REALSXP.0, n as c_int);
            let ra = REAL(ans);
            if n1 <= n2 {
                for i in 0..n {
...
unsafe fn rep2(s: SEXP, ncopy: SEXP) -> SEXP {
    unsafe {
        ...
        if TYPEOF(ncopy) != SEXPTYPE::INTSXP.0 {
            t = coerceVector(ncopy, SEXPTYPE::REALSXP.0);
        } else {
            t = coerceVector(ncopy, SEXPTYPE::INTSXP.0);
        }

        let mut sna: c_double = 0.0;
        if TYPEOF(t) == SEXPTYPE::REALSXP.0 {
            for i in 0..nc {
...
unsafe fn rep4(x: SEXP, times: SEXP, len: R_xlen_t, each: R_xlen_t, nt: R_xlen_t) -> SEXP {
    unsafe {
        ...
        } else if TYPEOF(times) == SEXPTYPE::REALSXP.0 {
    ...
    unsafe {
        ...
        if inherits(s, b"factor\0".as_ptr() as *const c_char) != 0 {
            if inherits(s, b"ordered\0".as_ptr() as *const c_char) != 0 {
                let tmp = Rf_allocVector(SEXPTYPE::STRSXP.0, 2);
...
pub unsafe fn do_rep_len(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        let na: R_xlen_t;
        if TYPEOF(len) != SEXPTYPE::INTSXP.0 {
            let sna = asReal(len);
        ...
        if TYPEOF(s) == SEXPTYPE::NILSXP.0 && na > 0 {
            return ptr::null_mut();
        }
...
pub unsafe fn do_rep(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        if TYPEOF(length_out_arg) != SEXPTYPE::INTSXP.0 {
        ...
        if TYPEOF(each_arg) != SEXPTYPE::INTSXP.0 {
        ...
        if len != NA_INTEGER as R_xlen_t {
            ...
            } else if TYPEOF(times_arg) != SEXPTYPE::INTSXP.0 {
                times = coerceVector(times_arg, SEXPTYPE::REALSXP.0);
            } else {
                times = coerceVector(times_arg, SEXPTYPE::INTSXP.0);
            }
            nt = XLENGTH(times);
            if nt == 1 {
                let it: R_xlen_t;
                if TYPEOF(times) == SEXPTYPE::REALSXP.0 {
                    let rt = *REAL(times);
...
pub unsafe fn do_seq(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        if one_arg && !miss_from {
            let lf = LENGTH(from);
            if lf == 1
                && (TYPEOF(from) == SEXPTYPE::INTSXP.0 || TYPEOF(from) == SEXPTYPE::REALSXP.0)
            {
                ...
                ans = Rf_allocVector(SEXPTYPE::INTSXP.0, 0);
            }
            return ans;
        }
        ...
        if lout == NA_INTEGER as R_xlen_t {
            ...
            if by == R_MissingArg() {
                ...
                if (!miss_from || TYPEOF(from) == SEXPTYPE::INTSXP.0)
                    && (!miss_to || TYPEOF(to) == SEXPTYPE::INTSXP.0)
                    && TYPEOF(by) == SEXPTYPE::INTSXP.0
                {
                    let nn = n as R_xlen_t;
                    ans = Rf_allocVector(SEXPTYPE::INTSXP.0, (nn + 1) as c_int);
                    let ia = INTEGER(ans);
                    ...
                    ans = Rf_allocVector(SEXPTYPE::REALSXP.0, (nn + 1) as c_int);
                    let ra = REAL(ans);
                    if finite_del {
                        for i in 0..=nn {
            ...
            {
                ans = Rf_allocVector(SEXPTYPE::INTSXP.0, lout as c_int);
                *INTEGER(ans) = rfrom as c_int;
                ...
                ans = Rf_allocVector(SEXPTYPE::REALSXP.0, lout as c_int);
                *REAL(ans) = rfrom;
                if lout > 1 {
...
pub unsafe fn do_sequence(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        let ans = Rf_allocVector(SEXPTYPE::INTSXP.0, ans_len as c_int);
...
mod tests {
    ...
    unsafe fn make_int_vec(vals: &[c_int]) -> SEXP {
        let v = Rf_allocVector(SEXPTYPE::INTSXP.0, vals.len() as c_int);
        let data = INTEGER(v);
    ...
    unsafe fn make_real_vec(vals: &[c_double]) -> SEXP {
        let v = Rf_allocVector(SEXPTYPE::REALSXP.0, vals.len() as c_int);
        let data = REAL(v);
        for (i, &val) in vals.iter().enumerate() {
            *data.add(i) = val;
        }
    ...
    fn test_seq_colon_simple_int_range() {
        unsafe {
            ...
            assert!(!ans.is_null());
            assert_eq!(TYPEOF(ans), SEXPTYPE::INTSXP.0);
    ...
    fn test_seq_colon_real_range() {
        unsafe {
            ...
            assert!(!ans.is_null());
            assert_eq!(TYPEOF(ans), SEXPTYPE::REALSXP.0);
    ...
    fn test_do_seq_along_empty() {
        unsafe {
            let x = Rf_allocVector(SEXPTYPE::INTSXP.0, 0);
            let args = Rf_cons(x, R_NilValue());
            let ans = do_seq_along(ptr::null_mut(), ptr::null_mut(), args, ptr::null_mut());
            assert!(!ans.is_null());
            assert_eq!(LENGTH(ans), 0);
    ...
    fn test_do_sequence_empty() {
        unsafe {
            let lengths = Rf_allocVector(SEXPTYPE::INTSXP.0, 0);
