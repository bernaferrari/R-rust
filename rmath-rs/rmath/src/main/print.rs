...
unsafe fn isDataFrame(x: SEXP) -> c_int {
    unsafe {
        if x.is_null() {
            return 0;
        }
...
unsafe fn GetMatrixDimnames(
    ...
    unsafe {
        crate::main::printvector::GetMatrixDimnames(x, rl, cl, rn, cn);
    }
}
...
unsafe fn allocArray(mode: c_int, dims: SEXP) -> SEXP {
    unsafe { crate::main::array::allocArray(mode, dims) }
}
...
pub unsafe fn PrintInit(data: *mut std::ffi::c_void, env: SEXP) {
    unsafe {
        ...
        (*d).na_width = crate::main::printutils::Rstrlen((*d).na_string, 0);
        (*d).na_width_noquote = crate::main::printutils::Rstrlen((*d).na_string_noquote, 0);
        (*d).quote = 1;
        (*d).right = Rprt_adj_left;
        (*d).digits = crate::main::options::GetOptionDigits();

        let scipen_sym = Rf_install(b"scipen\0".as_ptr() as *const c_char);
        (*d).scipen = asInteger(crate::main::options::GetOption1(scipen_sym));
        if (*d).scipen == NA_INTEGER {
            (*d).scipen = 0;
        }

        let maxprint_sym = Rf_install(b"max.print\0".as_ptr() as *const c_char);
        (*d).max = asInteger(crate::main::options::GetOption1(maxprint_sym));
        ...
        (*d).gap = 1;
        (*d).width = crate::main::options::GetOptionWidth();
        (*d).useSource = USESOURCE;
        (*d).cutoff = crate::main::options::GetOptionCutoff();
        (*d).env = env;
...
unsafe fn PrintClosure(s: SEXP, data: &R_PrintData) {
    unsafe {
        ...
        let t = CLOENV(s);
        if t != R_GlobalEnv() {
            let env_str = crate::main::printutils::EncodeEnvironment(t);
            if !env_str.is_null() {
                eprint!(
                    "{}\n",
    ...
    unsafe {
        ...
        if dims != R_NilValue() && LENGTH(dims) > 1 {
                    ...
                    match TYPEOF(s_i) {
                        ...
                        _t if _t == SEXPTYPE::LGLSXP.0 => {
                            if LENGTH(s_i) == 1 {
                                ...
                                if !x.is_null() {
                                    let mut w: c_int = 0;
                                    crate::main::format::formatLogical(x, 1, &mut w);
                                    let enc = crate::main::printutils::EncodeLogical(*x, w);
                                    if !enc.is_null() {
                        ...
                        _t if _t == SEXPTYPE::INTSXP.0 => {
                            if inherits_cstr(s_i, b"factor\0".as_ptr() as *const c_char) != 0 {
                                ...
                                if !x.is_null() {
                                    let mut w: c_int = 0;
                                    crate::main::format::formatInteger(x, 1, &mut w);
                                    let enc = crate::main::printutils::EncodeInteger(*x, w);
                                    if !enc.is_null() {
                                        let cs = CStr::from_ptr(enc);
                                        let bytes = cs.to_bytes();
                                        let copy_len = bytes.len().min(114);
                        ...
                        _t if _t == SEXPTYPE::REALSXP.0 => {
                            if LENGTH(s_i) == 1 {
                                ...
                                if !x.is_null() {
                                    ...
                                    let mut d: c_int = 0;
                                    let mut e: c_int = 0;
                                    crate::main::format::formatReal(
                                        x, 1, &mut w, &mut d, &mut e, 0,
                                    );
                                    let outdec: *const c_char = b".\0".as_ptr() as *const c_char;
                                    let enc =
                                        crate::main::printutils::EncodeReal0(*x, w, d, e, outdec);
                                    if !enc.is_null() {
                                        let cs = CStr::from_ptr(enc);
                                        let bytes = cs.to_bytes();
                                        let copy_len = bytes.len().min(114);
                            ...
                            if LENGTH(s_i) == 1 {
                                ...
                                if !x.is_null() {
                                    ...
                                    if R_IsNA(cx.r) || R_IsNA(cx.i) {
                                        ...
                                        let enc = crate::main::printutils::EncodeReal0(
                                            NA_REAL,
                                            data.na_width,
                                            0,
                                            0,
                                        ...
                                        let mut di: c_int = 0;
                                        let mut ei: c_int = 0;
                                        crate::main::format::formatComplex(
                                            x, 1, &mut wr, &mut dr, &mut er, &mut wi, &mut di,
                                            &mut ei, 0,
                                        );
                                        let outdec: *const c_char =
                                            b".\0".as_ptr() as *const c_char;
                                        let enc = crate::main::printutils::EncodeComplex(
            ...
            if LENGTH(dims) == 2 {
                ...
                crate::main::printarray::printMatrix(t, 0, dims, 0, data.right, rl, cl, rn, cn);
            } else {
                let names = Rf_protect(GetArrayDimnames(s));
                crate::main::printarray::printArray(t, dims, 0, Rprt_adj_left, names);
                Rf_unprotect(1);
            }
                            ...
                            if !name_chars.is_null() && *name_chars != 0 {
                                ...
                                if taglen + name_len > TAGBUFLEN {
                                    ...
                                    if name_elt == na_str {
                                        ...
                                        let enc = crate::main::printutils::EncodeChar(name_elt);
                                ...
                                let iw = crate::main::printutils::IndexWidth_xlen(i);
    ...
    unsafe {
        ...
        if dims != R_NilValue() && LENGTH(dims) > 1 {
            ...
            if LENGTH(dims) == 2 {
                ...
                crate::main::printarray::printArray(t, dims, 0, Rprt_adj_left, dimnames);
                Rf_unprotect(1);
            }
            Rf_unprotect(2); // dims, t
        } else {
                ...
                if tag != R_NilValue() && isSymbol(tag) != 0 {
                    ...
                    if taglen + name_len > TAGBUFLEN {
                        ...
                        if pname == na_str {
                            ...
                            let enc = crate::main::printutils::EncodeChar(pname);
                            if !enc.is_null() && isValidName(enc) {
                    ...
                    let iw = crate::main::printutils::IndexWidth_xlen(i as i64);
                    if taglen + iw as usize > TAGBUFLEN {
                        if taglen <= TAGBUFLEN {
                            libc::snprintf(ptag, sz, b"$...\0".as_ptr() as *const c_char);
                        }
...
unsafe fn PrintValueRec_inner(s: SEXP, data: &R_PrintData) {
    unsafe {
        ...
        if isMethodsDispatchOn() == 0 && (IS_S4_OBJECT(s) != 0 || TYPEOF(s) == SEXPTYPE::S4SXP.0) {
            eprint!("<S4 object>\n");
            return;
        }
        ...
        match TYPEOF(s) {
            ...
            t if t == SEXPTYPE::CHARSXP.0 => {
                eprint!("<CHARSXP: ");
                let enc = crate::main::printutils::EncodeString(
                    s,
                    0,
                    b'"' as c_int,
                    crate::main::printutils::Rprt_adj::none,
                );
                if !enc.is_null() {
            ...
            t if t == SEXPTYPE::ENVSXP.0 => {
                let env_str = crate::main::printutils::EncodeEnvironment(s);
                if !env_str.is_null() {
                    eprint!(
                        "{}\n",
                        CStr::from_ptr(env_str).to_str().unwrap_or("<environment>")
                ...
                if TYPEOF(dim) == SEXPTYPE::INTSXP.0 {
                    if LENGTH(dim) == 1 {
                        ...
                        if dnames != R_NilValue() && VECTOR_ELT(dnames, 0) != R_NilValue() {
                            ...
                            crate::main::printvector::printNamedVector(
                                s,
                                VECTOR_ELT(dnames, 0),
                                data.quote,
                                title,
                            ...
                            crate::main::printvector::printVector(s, 1, data.quote);
                        }
                        ...
                        crate::main::printarray::printArray(
        ...
        if cur_args != R_NilValue() {
            ...
            if Rf_isNull(naprint) == 0 {
                if isString(naprint) != 0 && LENGTH(naprint) >= 1 {
                    ...
                    data.na_width = crate::main::printutils::Rstrlen(data.na_string, 0);
...
pub unsafe fn do_unclass(_call: SEXP, _op: SEXP, args: SEXP, _env: SEXP) -> SEXP {
    unsafe {
        let x = CAR(args);
        if !x.is_null() {
            crate::attrib_core::setAttrib(x, R_ClassSymbol(), R_NilValue());
        }
        x
    }
}
...
unsafe fn GetOptionDigits() -> c_int {
    unsafe { crate::main::options::GetOptionDigits() }
}
...
mod tests {
    ...
    fn test_do_prmatrix_nil() {
        // do_prmatrix takes args as a pairlist; CAR(null) crashes.
        // Just verify the function exists.
        unsafe {
            let _ = (do_prmatrix as unsafe extern "C" fn(SEXP, SEXP, SEXP, SEXP) -> SEXP);
        }
    }
