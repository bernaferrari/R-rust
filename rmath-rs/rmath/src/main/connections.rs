...
unsafe fn inherits_class(x: SEXP, class_name: &str) -> bool {
    unsafe {
        ...
        let class_attr = crate::attrib_core::getAttrib(x, crate::attrib_core::R_ClassSymbol());
        if class_attr.is_null() || class_attr == R_NilValue() {
            return false;
        }
        if TYPEOF(class_attr) != SEXPTYPE::STRSXP.0 {
            return false;
        }
        let len = LENGTH(class_attr);
...
unsafe fn set_connection_class(ans: SEXP, specific_class: &str) {
    unsafe {
        ...
        crate::attrib_core::setAttrib(ans, class_sym, class_vec);
    }
}
...
mod tests {
    ...
    fn test_do_isopen() {
        ...
        unsafe {
            ...
            Rf_protect(rw_sxp);
            let args = Rf_cons(stdin_sxp, Rf_cons(rw_sxp, R_NilValue()));
            Rf_protect(args);
    ...
    fn test_conn_new_api() {
        ...
        unsafe {
            ...
            // Each guard must be dropped before acquiring the next (same mutex)
            {
                let _g = get_connection(0);
            }
            {
                let _g = get_connection(1);
            }
            {
                let _g = get_connection(2);
            }
        }
    }
