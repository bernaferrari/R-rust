...
unsafe fn isPairList(x: SEXP) -> bool {
    unsafe {
        let t = TYPEOF(x);
        t == SEXPTYPE::LISTSXP.0 || t == SEXPTYPE::NILSXP.0
    }
}

#[inline]
unsafe fn isList(x: SEXP) -> bool {
    unsafe { TYPEOF(x) == SEXPTYPE::LISTSXP.0 }
}
...
unsafe fn isInteger(x: SEXP) -> bool {
    unsafe { TYPEOF(x) == SEXPTYPE::INTSXP.0 }
}

#[inline]
unsafe fn isReal(x: SEXP) -> bool {
    unsafe { crate::main::coerce::isReal(x) }
}
...
#[inline]
unsafe fn IS_GROWABLE(x: SEXP) -> bool {
    unsafe { !x.is_null() && TRUELENGTH(x) > LENGTH(x) as c_int }
}

/// Set the growable bit on an object.
/// R uses negative truelength to indicate growable. We set truelength
/// to a negative value (R stores the growable flag via truelength sign).
#[inline]
unsafe fn SET_GROWABLE_BIT(x: SEXP) {
    unsafe {
        if !x.is_null() {
            let tl = TRUELENGTH(x);
            if tl >= 0 {
                // Set to negative to mark as growable (R convention)
                SET_TRUELENGTH(x, if tl > 0 { -tl } else { -1 });
            }
        }
    }
}
...
/// Delegates to the r-sexp implementation.
#[inline]
unsafe fn SET_TRUELENGTH(x: SEXP, v: c_int) {
    unsafe {
        crate::sexp::accessors::SET_TRUELENGTH(x, v);
    }
}
...
unsafe fn PairToVectorList(x: SEXP) -> SEXP {
    unsafe { crate::main::coerce::coerceVector(x, SEXPTYPE::VECSXP.0) }
}

/// VectorToPairList: convert a vector list to a pairlist.
unsafe fn VectorToPairList(x: SEXP) -> SEXP {
    unsafe { crate::main::coerce::coerceVector(x, SEXPTYPE::LISTSXP.0) }
}
...
unsafe fn copyMostAttrib(src: SEXP, dest: SEXP) {
    unsafe {
        crate::main::attrib::copyMostAttrib(src, dest);
    }
}
...
unsafe fn ScalarInteger(x: c_int) -> SEXP {
    unsafe {
        let s = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, 1));
        *INTEGER(s) = x;
        Rf_unprotect(1);
        s
    }
...
unsafe fn ScalarReal(x: c_double) -> SEXP {
    unsafe {
        let s = Rf_protect(Rf_allocVector3(SEXPTYPE::REALSXP.0, 1));
        *REAL(s) = x;
        Rf_unprotect(1);
        s
    }
...
unsafe fn ScalarString(x: SEXP) -> SEXP {
    unsafe {
        let s = Rf_protect(Rf_allocVector3(SEXPTYPE::STRSXP.0, 1));
...
unsafe fn fixSubset3Args(call: SEXP, args: SEXP, env: SEXP, nlist: *mut SEXP) -> SEXP {
    unsafe { crate::main::subset::fixSubset3Args(call, args, env, nlist) }
}

/// NonNullStringMatch: match non-null strings.
unsafe fn NonNullStringMatch(s: SEXP, t: SEXP) -> c_int {
    unsafe { crate::main::match_mod::NonNullStringMatch(s, t) }
}
...
unsafe fn NA_STRING() -> SEXP {
    unsafe { crate::main::relop::NA_STRING() }
}
...
unsafe fn DispatchOrEval(
    ...
    unsafe {
        crate::eval::dispatch::DispatchOrEval(call, op, generic, args, rho, ans, fallback, supplied)
    }
}

/// R_getS4DataSlot: get S4 data slot.
unsafe fn R_getS4DataSlot(x: SEXP, type_: c_int) -> SEXP {
    unsafe { crate::main::attrib::R_getS4DataSlot(x, type_) }
}
...
unsafe fn getNames(x: SEXP) -> SEXP {
    unsafe {
        ...
        use crate::attrib_core::R_NamesSymbol as R_NamesSym;
...
unsafe fn EnlargeVector(x: SEXP, newlen: R_xlen_t) -> SEXP {
    unsafe {
        ...
        let xtype = TYPEOF(x);
        if xtype == SEXPTYPE::LGLSXP.0 || xtype == SEXPTYPE::INTSXP.0 {
            let px = INTEGER(newx);
        ...
        } else if xtype == SEXPTYPE::CPLXSXP.0 {
            let px = COMPLEX(newx);
        ...
        } else if xtype == SEXPTYPE::EXPRSXP.0 || xtype == SEXPTYPE::VECSXP.0 {
            for i in 0..len {
        ...
        } else if xtype == SEXPTYPE::RAWSXP.0 {
            let px = RAW(newx);
            let px_src = RAW(x);
            for i in 0..len {
                *px.add(i as usize) = *px_src.add(i as usize);
...
unsafe fn EnlargeNames(names: SEXP, len: R_xlen_t, newlen: R_xlen_t) -> SEXP {
    unsafe {
        if TYPEOF(names) != SEXPTYPE::STRSXP.0 || XLENGTH(names) != len {
...
unsafe fn embedInVector(v: SEXP, _call: SEXP) -> SEXP {
    unsafe {
        let ans = Rf_protect(Rf_allocVector3(SEXPTYPE::VECSXP.0, 1));
...
unsafe fn SubassignTypeFix(
    ...
    unsafe {
        ...
        match which {
            ...
            1013 => {
                // logical <- integer
                *x = coerceVector(*x, SEXPTYPE::INTSXP.0);
            }
            ...
            1015 | 1315 | 1415 => {
                ...
                *x = coerceVector(*x, SEXPTYPE::CPLXSXP.0);
            }

            1610 | 1613 | 1614 | 1615 => {
                // character <- logical/integer/real/complex
                *y = coerceVector(*y, SEXPTYPE::STRSXP.0);
            }

            1016 | 1316 | 1416 | 1516 => {
                // logical/integer/real/complex <- character
                *x = coerceVector(*x, SEXPTYPE::STRSXP.0);
            }
            ...
            | 1922 | 1923 | 1924 | 1903 | 1907 | 1908 | 1999 => {
                // vector <- various
                if level == 1 {
                    *y = coerceVector(*y, SEXPTYPE::VECSXP.0);
                } else {
                    redo_which = false;
                }
            }
            ...
            1019 | 1319 | 1419 | 1519 | 1619 | 2419 => {
                // various <- vector
                *x = coerceVector(*x, SEXPTYPE::VECSXP.0);
            }

            1020 | 1320 | 1420 | 1520 | 1620 | 2420 => {
                // various <- expression
                *x = coerceVector(*x, SEXPTYPE::EXPRSXP.0);
            }
...
unsafe fn gi(indx: SEXP, i: R_xlen_t) -> R_xlen_t {
    unsafe {
        if TYPEOF(indx) == SEXPTYPE::REALSXP.0 {
...
unsafe fn DeleteListElements(x: SEXP, which: SEXP) -> SEXP {
    unsafe {
        ...
        let include = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, len));
        ...
        if !isNull(xnames) {
            let xnewnames = Rf_protect(Rf_allocVector3(SEXPTYPE::STRSXP.0, ii));
            k = 0;
            for i in 0..len {
                if *pinclude.add(i as usize) == 1 {
...
unsafe fn VectorAssign(call: SEXP, rho: SEXP, x: SEXP, s: SEXP, y: SEXP) -> SEXP {
    unsafe {
        ...
        if isNull(ATTRIB(s)) {
            if TYPEOF(x) == SEXPTYPE::REALSXP.0 && IS_SCALAR(y, SEXPTYPE::REALSXP.0) != 0 {
                if IS_SCALAR(s, SEXPTYPE::INTSXP.0) != 0 {
                    let ival = SCALAR_IVAL(s) as R_xlen_t;
                    if ival >= 1 && ival <= XLENGTH(x) {
                        *REAL(x).add((ival - 1) as usize) = SCALAR_DVAL(y);
                        return x;
                    }
                } else if IS_SCALAR(s, SEXPTYPE::REALSXP.0) != 0 {
        ...
        if (TYPEOF(x) != SEXPTYPE::VECSXP.0 && TYPEOF(x) != SEXPTYPE::EXPRSXP.0) || !isNull(y) {
...
unsafe fn listRemove(x: SEXP, s: SEXP, ind: c_int) -> SEXP {
    unsafe {
        ...
        if TYPEOF(s) == SEXPTYPE::REALSXP.0 {
            for i in 0..ns {
                let di = REAL_ELT(s, i);
                if R_FINITE(di) {
...
unsafe fn DeleteOneVectorListItem(x: SEXP, which: R_xlen_t) -> SEXP {
    unsafe {
        ...
        if which >= 0 && which < n {
            ...
            let xnames = Rf_protect(getAttrib(x, R_NamesSym()));
            if !isNull(xnames) {
                let ynames = Rf_protect(Rf_allocVector3(SEXPTYPE::STRSXP.0, n - 1));
                k = 0;
                for i in 0..n {
                    if i != which {
...
unsafe fn errorNotSubsettable(x: SEXP) {
    unsafe {
        ...
        let type_name = if x.is_null() {
            "NULL"
        } else {
            match TYPEOF(x) {
                t if t == SEXPTYPE::CLOSXP.0 => "closure",
                t if t == SEXPTYPE::ENVSXP.0 => "environment",
                t if t == SEXPTYPE::EXTPTRSXP.0 => "external pointer",
                t if t == SEXPTYPE::WEAKREFSXP.0 => "weak reference",
                t if t == SEXPTYPE::SPECIALSXP.0 || t == SEXPTYPE::BUILTINSXP.0 => "function",
                _ => "object",
            }
        };
        std::panic::panic_any(crate::sexp::context::RError {
            message: format!("object of type '{}' is not subsettable", type_name),
        });
    }
}
...
/// In C this is NORET (never returns).
unsafe fn errorOutOfBoundsSEXP(x: SEXP, subscript: c_int, sindex: SEXP) {
    unsafe {
        use crate::sexp::accessors::{CHAR, LENGTH, STRING_ELT};
        use std::ffi::CStr;
        let msg = if subscript < 0 {
            // Missing subscript
            if !sindex.is_null() && TYPEOF(sindex) == SEXPTYPE::STRSXP.0 && LENGTH(sindex) >= 1 {
                ...
                if !elt.is_null() {
                    ...
                    if !cs.is_null() {
                        format!(
                            ...
                            CStr::from_ptr(cs).to_str().unwrap_or("?")
                        )
                    } else {
                        "subscript out of bounds".to_string()
                    }
                } else {
                    "subscript out of bounds".to_string()
                }
            ...
            format!("subscript out of bounds")
        };
...
pub unsafe fn do_subassign_dflt(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        if TYPEOF(x) == SEXPTYPE::LISTSXP.0 || TYPEOF(x) == SEXPTYPE::LANGSXP.0 {
            oldtype = TYPEOF(x);
            ...
            if XLENGTH(y) == 0
                && (isNull(x)
                    || TYPEOF(x) == TYPEOF(y)
                    || TYPEOF(y) == SEXPTYPE::VECSXP.0
                    || TYPEOF(y) == SEXPTYPE::EXPRSXP.0)
            {
                Rf_unprotect(2);
                return x;
            } else {
        ...
        if oldtype == SEXPTYPE::LANGSXP.0 {
            if Rf_length(x) > 0 {
                x = VectorToPairList(x);
                SET_TYPEOF(x, SEXPTYPE::LANGSXP.0);
            }
        }
...
pub unsafe fn do_subassign2_dflt(call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        let _ = op;
        use crate::attrib_core::R_DimNamesSymbol;
        use crate::attrib_core::R_DimSymbol;
        use crate::attrib_core::R_NamesSymbol as R_NamesSym;
        use crate::sexp::globals::R_MissingArg;
        ...
        if isNull(x) {
            ...
            x = Rf_allocVector3(SEXPTYPE::VECSXP.0, 0);
        }
        ...
        let s4 = IS_S4_OBJECT(x);
        let xOrig = if s4 != 0 && TYPEOF(x) == SEXPTYPE::OBJSXP.0 {
            let orig = x;
            x = R_getS4DataSlot(x, SEXPTYPE::ANYSXP.0);
            orig
        } else {
            ptr::null_mut()
        };
        ...
        let pdims: *const c_int = if ndims > 0 {
            if TYPEOF(dims) == SEXPTYPE::INTSXP.0 {
                INTEGER(dims)
            } else {
                ptr::null()
            }
        ...
        if isVector(x) {
            ...
            if nsubs == 1 {
                ...
                let indx = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, ndims as R_xlen_t));
            ...
            if stretch > 0 && !isNull(newname) {
                ...
                if isNull(names) {
                    let names_new = Rf_protect(Rf_allocVector3(
                        SEXPTYPE::STRSXP.0,
                        Rf_length(x) as R_xlen_t,
                    ));
...
pub unsafe fn R_subassign3_dflt(call: SEXP, x: SEXP, nlist: SEXP, val: SEXP) -> SEXP {
    unsafe {
        ...
        } else if TYPEOF(x) == SEXPTYPE::ENVSXP.0 {
            defineVar(nlist, val, x);
            INCREMENT_NAMED(val);
        } else if TYPEOF(x) == SEXPTYPE::SYMSXP.0
            || TYPEOF(x) == SEXPTYPE::CLOSXP.0
            ...
            || TYPEOF(x) == SEXPTYPE::BUILTINSXP.0
        {
            ...
            let nx = XLENGTH(x);
            let mut atype = SEXPTYPE::VECSXP.0;

            if isExpression(x) {
                atype = SEXPTYPE::EXPRSXP.0;
            } else if !isNewList(x) {
                x = coerceVector(x, SEXPTYPE::VECSXP.0);
            }
            ...
            if isNull(val) {
                ...
                if !isNull(names) {
                    ...
                    if imatch >= 0 {
                        ...
                        let ansnames = Rf_allocVector3(SEXPTYPE::STRSXP.0, nx - 1);
                        let mut ii: R_xlen_t = 0;
                        for i in 0..nx {
                ...
                if imatch >= 0 {
                    ...
                    // Add new element
                    let ans = Rf_allocVector3(SEXPTYPE::VECSXP.0, nx + 1);
                    let ansnames = Rf_allocVector3(SEXPTYPE::STRSXP.0, nx + 1);
                    for i in 0..nx {
...
mod tests {
    ...
    fn test_SubAssignArgs_two_args() {
        unsafe {
            ...
            let y_val = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, 1));
    ...
    fn test_SubassignTypeFix_same_type() {
        unsafe {
            let mut xv: SEXP = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, 1));
            let mut yv: SEXP = Rf_protect(Rf_allocVector3(SEXPTYPE::INTSXP.0, 1));
