...
/// IS_BYTES check — checks the BYTES gp bit.
#[inline(always)]
unsafe fn IS_BYTES(s: SEXP) -> c_int {
    unsafe {
        ...
        let gp = (*s).sxpinfo.gp();
        if (gp & 0x02) != 0 { 1 } else { 0 } // GP_BYTES_MASK = 1 << 1
    }
}

/// ENC_KNOWN check — checks if encoding flag is set.
#[inline(always)]
...
/// IS_CACHED check — always returns true (no caching in this port).
#[inline(always)]
...
unsafe fn getCharCE(s: SEXP) -> c_int {
    unsafe {
        ...
        // CE_UTF8 (GP_UTF8_MASK = 1 << 3)
        else if (gp & 0x04) != 0 {
            2
        }
        // CE_LATIN1 (GP_LATIN1_MASK = 1 << 2)
        else if (gp & 0x02) != 0 {
            3
        }
        // CE_BYTES (GP_BYTES_MASK = 1 << 1)
        else {
            0
        } // CE_NATIVE
    }
