...
/// UNIMPLEMENTED_TYPE: signal an error for unimplemented types.
unsafe fn UNIMPLEMENTED_TYPE(routine: *const c_char, s: SEXP) {
    unsafe {
        use crate::sexp::accessors::TYPEOF;
        use std::ffi::CStr;
        let routine_str = if !routine.is_null() {
            CStr::from_ptr(routine).to_str().unwrap_or("???")
        } else {
            "???"
        };
        let type_str = format!("type {}", TYPEOF(s));
        std::panic::panic_any(crate::sexp::context::RError {
            message: format!("unimplemented type '{}' in '{}'", type_str, routine_str),
        });
    }
}
...
mod tests {
    ...
    fn test_duplicate_closure() {
        unsafe {
            ...
            let c = crate::main::dstruct::mkCLOSXP(formals, body, env);
            let d = duplicate(c);
            assert!(!d.is_null());
