...
unsafe fn isVectorList(x: SEXP) -> c_int {
    unsafe {
        ...
        let t = TYPEOF(x);
        (t == VECSXP_VAL || t == EXPRSXP_VAL) as c_int
    }
}
...
unsafe fn isFunction(x: SEXP) -> c_int {
    unsafe {
        ...
        let t = TYPEOF(x);
        (t == CLOSXP_VAL || t == SPECIALSXP_VAL || t == BUILTINSXP_VAL) as c_int
    }
}
...
unsafe fn isFactor(x: SEXP) -> c_int {
    unsafe {
        ...
        if TYPEOF(klass) != STRSXP_VAL || LENGTH(klass) == 0 {
            return 0;
        }
...
unsafe fn checkArgIsSymbol(x: SEXP) -> SEXP {
    unsafe {
        if x.is_null() || TYPEOF(x) != SYMSXP_VAL {
...
pub unsafe fn do_vapply(_call: SEXP, op: SEXP, args: SEXP, rho: SEXP) -> SEXP {
    unsafe {
        ...
        if commonType != CPLXSXP_VAL
            && commonType != REALSXP_VAL
            && commonType != INTSXP_VAL
            && commonType != LGLSXP_VAL
            && commonType != RAWSXP_VAL
            && commonType != STRSXP_VAL
            && commonType != VECSXP_VAL
        {
        ...
        if useNames != 0 {
            ...
            if Rf_isNull(names) != 0 && TYPEOF(XX) == STRSXP_VAL {
                names = XX;
            }
            rowNames = getAttrib(
                value,
        ...
        for i in 0..n {
            ...
            if valType != commonType {
                ...
                match commonType {
                    t if t == CPLXSXP_VAL => {
                        okay = valType == REALSXP_VAL
                            || valType == INTSXP_VAL
                            || valType == LGLSXP_VAL;
                    }
                    t if t == REALSXP_VAL => {
                        okay = valType == INTSXP_VAL || valType == LGLSXP_VAL;
                    }
                    t if t == INTSXP_VAL => {
...
pub(crate) unsafe fn islistfactor_recursive(x: SEXP) -> c_int {
    unsafe {
        ...
        match t {
            t if t == VECSXP_VAL || t == EXPRSXP_VAL => {
                let n = LENGTH(x);
...
mod tests {
    ...
    fn test_do_islistfactor_null() {
        unsafe {
            ...
            assert!(!result.is_null());
            assert_eq!(TYPEOF(result), LGLSXP_VAL);
