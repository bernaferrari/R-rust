...
use crate::sexp::accessors::{
    ATTRIB, CAR, CDR, CHAR, LENGTH, OBJECT, PRIMOFFSET, PRINTNAME, SET_ATTRIB, SET_INTERNAL,
    SET_PRINTNAME, SET_SYMVALUE, STRING_ELT, TYPEOF,
};
use crate::sexp::constructors::{Rf_allocVector, Rf_cons, Rf_isNull, Rf_mkChar, Rf_mkString};
...
unsafe fn installFunTab(i: usize) {
    unsafe {
        ...
        let name_cstr = std::ffi::CString::new(entry_name).unwrap();
    ...
    unsafe {
        let symbols = DDVAL_SYMBOLS.get_or_init(|| {
            ...
            for i in 0..N_DDVAL_SYMBOLS {
                let name = format!("..{}", i);
                let sym = Rf_install(std::ffi::CString::new(name).unwrap().as_ptr());
                v.push(sym);
            }
            DDVALSymbolsInner(v)
        });
