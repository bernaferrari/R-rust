...
//! R's interpreter uses everywhere. Implemented as static OnceLock values
...
// ---------------------------------------------------------------------------
// Sentinel singletons (using OnceLock for stable addresses)
// ---------------------------------------------------------------------------

/// R_NilValue: the global nil/NULL object.
static NIL_VALUE: OnceLock<SexprecCore> = OnceLock::new();

/// R_UnboundValue: sentinel for unbound symbols in environments.
static UNBOUND_VALUE: OnceLock<SexprecCore> = OnceLock::new();

/// R_MissingArg: sentinel for missing function arguments.
static MISSING_ARG: OnceLock<SexprecCore> = OnceLock::new();

/// R_RestartToken: sentinel for restart tokens.
static RESTART_TOKEN: OnceLock<SexprecCore> = OnceLock::new();

// ---------------------------------------------------------------------------
// Initialization
// ---------------------------------------------------------------------------

/// Initialize the global singletons. Called once at startup.
fn init_nil() -> &'static SexprecCore {
    NIL_VALUE.get_or_init(|| SexprecCore {
...
fn init_unbound() -> &'static SexprecCore {
...
fn init_missing() -> &'static SexprecCore {
...
fn init_restart() -> &'static SexprecCore {
...
pub unsafe extern "C" fn R_RestartToken() -> SEXP {
    let val = init_restart();
    val as *const _ as SEXP
}

// ---------------------------------------------------------------------------
// Global environment pointers (using AtomicPtr for thread safety)
// ---------------------------------------------------------------------------

static R_GLOBAL_ENV_PTR: AtomicPtr<SexprecCore> = AtomicPtr::new(ptr::null_mut());
static R_BASE_ENV_PTR: AtomicPtr<SexprecCore> = AtomicPtr::new(ptr::null_mut());
static R_EMPTY_ENV_PTR: AtomicPtr<SexprecCore> = AtomicPtr::new(ptr::null_mut());
static R_BASE_NAMESPACE_PTR: AtomicPtr<SexprecCore> = AtomicPtr::new(ptr::null_mut());
...
static R_TRUE: OnceLock<SexprecCore> = OnceLock::new();

/// R_False: the logical FALSE singleton (scalar LGLSXP with value 0).
static R_FALSE: OnceLock<SexprecCore> = OnceLock::new();
