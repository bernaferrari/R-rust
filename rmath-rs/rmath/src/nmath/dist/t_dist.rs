...
pub fn dt_inner(x: f64, n: f64, give_log: bool) -> f64 {
    ...
    if !r_finite(n) {
        return crate::nmath::dist::normal::dnorm4_inner(x, 0.0, 1.0, give_log);
    }

    let t = -crate::nmath::special::bd0::bd0(n / 2.0, (n + 1.0) / 2.0)
        + crate::nmath::special::stirlerr::stirlerr((n + 1.0) / 2.0)
        - crate::nmath::special::stirlerr::stirlerr(n / 2.0);
    ...
    if lrg_x2n {
        ...
        l_x2n = log1p(x2n) / 2.0;
        u = -crate::nmath::special::bd0::bd0(n / 2.0, (n + x * x) / 2.0) + x * x / 2.0;
    }
...
pub fn pt_inner(x: f64, n: f64, lower_tail: bool, log_p: bool) -> f64 {
    ...
    if !r_finite(n) {
        return crate::nmath::dist::normal::pnorm5_inner(x, 0.0, 1.0, lower_tail, log_p);
    }
    ...
    if nx > 1e100 {
        ...
        let lbeta_val = crate::nmath::special::gamma::lgammafn(0.5 * n)
            + crate::nmath::special::gamma::lgammafn(0.5)
            - crate::nmath::special::gamma::lgammafn(0.5 * n + 0.5);
        let lval = -0.5 * n * (2.0 * log(fabs(x)) - log(n)) - lbeta_val - log(0.5 * n);
        val = if log_p { lval } else { exp(lval) };
    } else if n > x * x {
        val = crate::nmath::dist::beta::pbeta_inner(
            x * x / (n + x * x),
            0.5,
            n / 2.0,
            false, /* lower_tail = 0 */
...
pub fn qt_inner(p: f64, ndf: f64, lower_tail: bool, log_p: bool) -> f64 {
    ...
    if ndf < 1.0 {
        ...
        if iter >= 1000 {
            crate::nmath::error::ml_warning(ME_PRECISION, "qt");
        }
    ...
    /* ndf >= 1 */
    if ndf > 1e20 {
        return crate::nmath::dist::normal::qnorm5_inner(p, 0.0, 1.0, lower_tail, log_p);
    }
    ...
    if fabs(ndf - 2.0) < eps {
        ...
        if (ndf < 2.1 && P > 0.5) || y > 0.05 + a {
            ...
            if p_ok {
                x = crate::nmath::dist::normal::qnorm5_inner(
                    0.5 * P,
                    0.0,
                    1.0,
                    true,  /* lower_tail */
...
pub fn rt_inner(df: f64) -> f64 {
    ...
    if !r_finite(df) {
        return crate::nmath::dist::normal::norm_rand();
    } else {
        let num = crate::nmath::dist::normal::norm_rand();
        num / sqrt(crate::nmath::dist::chisq::rchisq_inner(df) / df)
    }
}
...
#[must_use]
pub fn Rf_dt(x: f64, n: f64, give_log: i32) -> f64 {
    dt_inner(x, n, give_log != 0)
}

#[must_use]
pub fn dt(x: f64, n: f64, give_log: i32) -> f64 {
    dt_inner(x, n, give_log != 0)
}

#[must_use]
pub fn Rf_pt(x: f64, n: f64, lower_tail: i32, log_p: i32) -> f64 {
...
#[must_use]
pub fn Rf_qt(p: f64, ndf: f64, lower_tail: i32, log_p: i32) -> f64 {
    qt_inner(p, ndf, lower_tail != 0, log_p != 0)
}

#[must_use]
pub fn qt(p: f64, ndf: f64, lower_tail: i32, log_p: i32) -> f64 {
    qt_inner(p, ndf, lower_tail != 0, log_p != 0)
}

#[must_use]
pub fn Rf_rt(df: f64) -> f64 {
    rt_inner(df)
}

#[must_use]
pub fn rt(df: f64) -> f64 {
    rt_inner(df)
}
