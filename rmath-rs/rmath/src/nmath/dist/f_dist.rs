...
fn dbinom_raw(x: f64, n: f64, p: f64, q: f64, give_log: bool) -> f64 {
    ...
    if !r_finite(n) {
        if r_finite(x) {
            ...
            let n = DBL_MAX;
            let lc = crate::nmath::special::stirlerr::stirlerr(n)
                - crate::nmath::special::stirlerr::stirlerr(x)
                - crate::nmath::special::stirlerr::stirlerr(n - x)
                - crate::nmath::special::bd0::bd0(x, n * p)
                - crate::nmath::special::bd0::bd0(n - x, n * q);
...
pub fn df_inner(x: f64, m: f64, n: f64, give_log: bool) -> f64 {
    ...
    if !r_finite(n) {
        // must be +Inf by now
        return crate::nmath::dist::gamma::dgamma_inner(x, m / 2.0, 2.0 / m, give_log);
    }
    if m > 1e14 {
        // includes +Inf: code below is inaccurate there
        let dens = crate::nmath::dist::gamma::dgamma_inner(1.0 / x, n / 2.0, 2.0 / n, give_log);
        return if give_log {
            dens - 2.0 * log(x)
        } else {
            dens / (x * x)
...
pub fn pf_inner(x: f64, df1: f64, df2: f64, lower_tail: bool, log_p: bool) -> f64 {
    ...
    if df2 == ML_POSINF {
        ...
        return crate::nmath::dist::chisq::pchisq_inner(x * df1, df1, lower_tail, log_p);
    }

    if df1 == ML_POSINF {
        /* was "fudge" 'df1 > 4e5' in 2.0.x */
        return crate::nmath::dist::chisq::pchisq_inner(df2 / x, df2, !lower_tail, log_p);
    }

    /* Avoid squeezing pbeta's first parameter against 1 : */
    if df1 * x > df2 {
        x = crate::nmath::dist::beta::pbeta_inner(
            df2 / (df2 + df1 * x),
            df2 / 2.0,
            df1 / 2.0,
            !lower_tail,
...
pub fn qf_inner(p: f64, df1: f64, df2: f64, lower_tail: bool, log_p: bool) -> f64 {
    ...
    if df1 <= df2 && df2 > 4e5 {
        ...
        return crate::nmath::dist::chisq::qchisq_inner(p, df1, lower_tail, log_p) / df1;
    } else if df1 > 4e5 {
...
pub fn rf_inner(n1: f64, n2: f64) -> f64 {
    ...
    let v1 = if r_finite(n1) {
        crate::nmath::dist::chisq::rchisq_inner(n1) / n1
    } else {
        1.0
    };
    let v2 = if r_finite(n2) {
        crate::nmath::dist::chisq::rchisq_inner(n2) / n2
    } else {
        1.0
    };
    v1 / v2
