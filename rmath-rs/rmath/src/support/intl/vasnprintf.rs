    ...
    unsafe {
        ...
        while *cp != 0 {
            if *cp == b'%' as c_char {
                ...
                // Parse flags.
                let mut _flags_left = false;
                let mut _flags_plus = false;
                let mut _flags_space = false;
                let mut _flags_zero = false;
                let mut _flags_alt = false;
                ...
                // Parse precision.
                let mut _precision: c_int = -1;
                if *cp == b'.' as c_char {
                    cp = cp.add(1);
                    _precision = 0;
                    while *cp >= b'0' as c_char && *cp <= b'9' as c_char {
                        _precision = _precision * 10 + (*cp as c_int - b'0' as c_int);
                        cp = cp.add(1);
                    }
                }

                // Parse length modifier.
                let mut _is_long = false;
                let mut _is_long_long = false;
                loop {
                    match *cp {
