...
fn tzload(name: Option<&str>, sp: &mut state, doextend: bool) -> i32 {
    ...
    // Build the full path
    let name_to_use: &str;
    let _fullname_holder: String;

    if name_stripped.starts_with('/') {
        name_to_use = name_stripped;
        _fullname_holder = name_stripped.to_string();
    } else {
        ...
        _fullname_holder = format!("{}/{}", zoneinfo_path, name_stripped);
        name_to_use = &_fullname_holder;
    }
...
fn localsub(g: &mut TzGlobals, timep: &i64, _offset: i32, tmp: &mut stm) -> Option<*mut stm> {
    ...
    {
        let ats_first = ats_first.unwrap_or_default();
        let ats_last = ats_last.unwrap_or_default();
        let mut newt = t;
