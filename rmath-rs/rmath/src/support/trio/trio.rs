...
pub use crate::support::trio::trionan::*;
pub use crate::support::trio::triostr;
...
/// Unsigned long long.
type trio_ulonglong_t = u64;

/// Signed long long.
type trio_longlong_t = i64;
...
const TRIO_EOF: c_int = 1;
const TRIO_EINVAL: c_int = 2;
const TRIO_ETOOMANY: c_int = 3;
const TRIO_EDBLREF: c_int = 4;
const TRIO_EGAP: c_int = 5;
const TRIO_ENOMEM: c_int = 6;
const TRIO_ERANGE: c_int = 7;
const TRIO_ERRNO: c_int = 8;
const TRIO_ECUSTOM: c_int = 9;
...
fn trio_write_string<W: Write>(
    ...
    let (_s, len): (*const c_char, usize) = if string.is_null() {
...
fn trio_write_double<W: Write>(
    ...
    {
        let mut neg: c_int = 0;
        let class = crate::support::trio::trionan::trio_fpclassify_and_signbit(number, &mut neg);
        is_negative = neg != 0;
        match class {
            TRIO_FP_NAN => {
    ...
    loop {
        ...
        if flags & FLAGS_WIDTH_PARAMETER != 0 {
            ...
            if width < 0 {
                ...
                let _adj_flags = flags | FLAGS_LEFTADJUST & !FLAGS_NILPADDING;
                width = -width;
                ...
                if got {
                    ...
                    for &expected in nil_str.iter() {
                        if *input_pos < input.len() && input[*input_pos] == expected {
                            *input_pos += 1;
                            let _ch = if *input_pos < input.len() {
                                input[*input_pos] as c_int
                            } else {
                                -1
                            };
...
pub unsafe fn trio_string_contains(
    string: *const c_char,
