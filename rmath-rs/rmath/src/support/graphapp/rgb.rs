...
//! RGB colour name table and colour manipulation functions.
//!
...
const RGBCOLORS: usize = 657;

static ColorName: &[&str] = &[
    "AliceBlue",
    "AntiqueWhite",
    "AntiqueWhite1",
    "AntiqueWhite2",
...
fn strcmpi(s1: &str, s2: &str) -> i32 {
    let mut chars1 = s1.chars();
    let mut chars2 = s2.chars();
    loop {
        let c1 = chars1.next().map(|c| c.to_ascii_lowercase() as i32);
        let c2 = chars2.next().map(|c| c.to_ascii_lowercase() as i32);
        match (c1, c2) {
            (Some(a), Some(b)) => {
                let diff = a - b;
                if diff != 0 {
...
pub unsafe fn nametorgb(name: *const c_char) -> rgb {
    unsafe {
        ...
        let s = match CStr::from_ptr(name).to_str() {
            Ok(s) => s,
            Err(_) => return Transparent,
        };

        let mut low: i32 = 0;
        let mut high: i32 = (RGBCOLORS - 1) as i32;
        let mut mid: i32 = 0;

        while low <= high {
            mid = (low + high) / 2;
            let cmp = strcmpi(s, ColorName[mid as usize]);
            if cmp < 0 {
                high = mid - 1;
            } else if cmp > 0 {
                low = mid + 1;
        ...
        if high < low {
            ...
            let v = &RgbValue[mid as usize];
            rgb_make(v[0] as c_ulong, v[1] as c_ulong, v[2] as c_ulong)
        }
    }
}
...
pub fn rgb_name(in_val: rgb) -> Option<&'static str> {
...
/// Compatibility spelling for GraphApp callers.
pub fn rgbtoname(in_val: rgb) -> Option<&'static str> {
    rgb_name(in_val)
}
...
pub unsafe fn darker(pixel: rgb) -> rgb {
    ...
    let b = getblue(pixel);
    rgb_make((r + 1) * 3 / 4, (g + 1) * 3 / 4, (b + 1) * 3 / 4)
}
...
pub unsafe fn brighter(pixel: rgb) -> rgb {
    ...
    let g = getgreen(pixel);
    let b = getblue(pixel);
    let r = ((r as c_ulong) * 4 / 3).min(255);
...
mod tests {
    ...
    #[test]
    fn rgbtoname_returns_borrowed_table_name() {
        assert_eq!(rgbtoname(Black), Some("black"));
        assert_eq!(rgbtoname(Transparent), None);
    }
}
