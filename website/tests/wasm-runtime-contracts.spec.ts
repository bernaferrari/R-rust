...
test("GNU persistence hooks restore records and survive callback GC", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-persistence-hooks/persistent-token.rds",
      import.meta.url
    )
  )
...
test("GNU SWITCH executes modified branches and rejects malformed constants", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
  ...
  ].forEach((word, index) => stream.writeInt32BE(word, index * 4))
...
test("GNU DUP executes its stream in the browser", async ({ page }) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-dup/duplicate.rds",
      import.meta.url
    )
  )
  const stream = Buffer.alloc(28)
  ;[12, 20, 1, 5, 44, 0, 1].forEach((word, index) =>
    stream.writeInt32BE(word, index * 4)
  )
  const offset = bytes.indexOf(stream)
  expect(offset).toBeGreaterThan(-1)
  ...
  const output = await page.evaluate(async (values) => {
    ...
    try {
      return (
        await runtime.run(
          `f<-unserialize(as.raw(c(${values})));identical(f(3L),9L)`,
          "console"
        )
      ).output.trim()
    } finally {
...
test("match.call preserves S3 source expressions and matches formal arguments", async ({
  page,
}) => {
  await page.goto("/console/")
...
test("namespace bytecode, automatic file opening and sink routing match GNU", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-checkfun/base-abs.rds",
      import.meta.url
    )
  )
  await page.goto("/console/")
...
test("GNU computed calls and deferred capture connections work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-checkfun/callable-argument.rds",
      import.meta.url
    )
  )
  await page.goto("/console/")
...
test("GNU primitive calls and streaming file capture work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-builtin-calls/nested.rds",
      import.meta.url
    )
  )
  await page.goto("/console/")
...
test("literal GNU call arguments, QR transforms and selective capture work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-constant-args/pushconstarg.rds",
      import.meta.url
    )
  )
  const stream = Buffer.alloc(40)
  ;[12, 23, 1, 34, 2, 29, 3, 38, 0, 1].forEach((word, index) =>
    stream.writeInt32BE(word, index * 4)
  )
  const offset = bytes.indexOf(stream)
  expect(offset).toBeGreaterThan(-1)
...
test("GNU superassignment, evalq, Q factors and output capture work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-assignment/setvar2.rds",
      import.meta.url
    )
  )
  await page.goto("/console/")
...
test("named GNU calls, real QR and method existence work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-named-calls/nested-reversed-tags.rds",
      import.meta.url
    )
  )
  await page.goto("/console/")
...
test("compiled lazy calls and inherited method specificity work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-calls/identity-promise.rds",
      import.meta.url
    )
  )
...
test("GNU math bytecode, method continuation and abort discovery work in Wasm", async ({
  ...
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-red-compiler/sqrt.rds",
      import.meta.url
    )
  )
  const stream = Buffer.alloc(36)
  ;[12, 123, 0, 8, 20, 1, 49, 0, 1].forEach((word, i) =>
    stream.writeInt32BE(word, i * 4)
  )
  const offset = bytes.indexOf(stream)
  expect(offset).toBeGreaterThan(-1)
...
test("unary GNU bytecode, primitive methods and restart unwinding work in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
      "../../crates/r-embed/tests/fixtures/gnu-bytecode-unary/negative.rds",
      import.meta.url
    )
  )
...
test("GNU arithmetic bytecode and Ops conflict hooks run in Wasm", async ({
  page,
}) => {
  const bytes = readFileSync(
    new URL(
  ...
  ;[12, 20, 1, 20, 2, 44, 0, 1].forEach((word, i) =>
    stream.writeInt32BE(word, i * 4)
  )
  const offset = bytes.indexOf(stream)
  expect(offset).toBeGreaterThan(-1)
