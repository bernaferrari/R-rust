...
test("autoruns the initial LOESS example", async ({ page }) => {
  await page.goto("/")
...
test("stops a running R worker and clears its session", async ({ page }) => {
  ...
  await expect(page.locator("#playground [role=status]")).toContainText(
    "Session cleared"
  )
  await expect(
    page.getByRole("button", { name: "Stop", exact: true })
...
test("filters the gallery, handles an empty search, and selects a recipe", async ({
  ...
  await search.fill("averages settle")
  await page.locator(".example-card").click({ position: { x: 80, y: 50 } })
  await expect(
    page.getByRole("combobox", { name: "Choose an R example" })
  ).toContainText("Watch averages settle")
  await expect(page.locator(".cm-content")).toContainText("central limit theorem")
  await expect(page.getByAltText("Plot generated by your R code")).toBeVisible({
    timeout: 30_000,
  })
})
...
test("drafts through a mocked Ollama endpoint and hands code to the playground", async ({
  ...
  await page.getByRole("button", { name: "Run in playground" }).click()
