...
This repository includes upstream GNU R source under `r-source/` for source
comparison, conformance, and attribution. GNU R is distributed under the GNU
General Public License; see `r-source/COPYING` and the upstream notices in that
tree. Translated or behaviorally derived Rust modules should keep their upstream
source anchors in `docs/upstream-port-map.tsv`.
