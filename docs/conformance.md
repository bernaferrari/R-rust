...
| --- | ---: | --- |
| Parser and scalar basics | 182 | Arithmetic, scalar values, comments, infix continuation, parse/deparse/dput/bquote/RDS/unname/expression/mode/storage-mode/tsp/comment-attribute/attr/attributes/dim/length-replacement, repetition helpers, tabulation, findInterval boundary options, shape-helper, array-creation, and broad parser/runtime smoke cases |
...
| Stats, math, and RNG | 73 | Numeric summaries, distributions, tail/log flags, arithmetic edge cases, complex hyperbolics, `sample`/`sample.int`, pmin/pmax missing-value semantics, typed cumulative extrema, typed cumulative sum/product NA semantics, array margin summaries, aggregate/tapply/by grouped summaries, and summary-vector parity |
