...
| Base functions, conditions, and platform helpers | 78 | Sorting/set helpers, output capture, conditions, `ls`, `system`/`system2`, `Sys.getenv`/`Sys.setenv`, `proc.time`, file/temp helpers including `file.info`, `cat(file=...)`, and path-string `readLines`, wrapping helpers |
